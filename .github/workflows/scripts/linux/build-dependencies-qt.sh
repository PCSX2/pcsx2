...
if [ "$BUILD_LIBRASHADER" -ne 0 ]; then
	...
	cargo run -p librashader-build-script -- --profile optimized --stable -- --no-default-features --features runtime-opengl,runtime-vulkan
	...
	if [ -f "target/optimized/librashader.so" ]; then
		install -m 0755 "target/optimized/librashader.so" "$INSTALLDIR/lib/liblibrashader.so"
	elif [ -f "target/optimized/liblibrashader.so" ]; then
		install -m 0755 "target/optimized/liblibrashader.so" "$INSTALLDIR/lib/liblibrashader.so"
	else
		echo "librashader shared library not found"
		exit 1
	fi
	patchelf --set-soname liblibrashader.so "$INSTALLDIR/lib/liblibrashader.so"
	cd ..
else
