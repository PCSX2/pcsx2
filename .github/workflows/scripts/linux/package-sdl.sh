#!/usr/bin/env bash
#
# package-sdl.sh — assemble a self-contained armsx2-sdl handheld tarball.
#
# armsx2: the SDL3 frontend is a bare-kmsdrm handheld build (VK_KHR_display
...
#
#   <name>/
#     armsx2-sdl            (the frontend, rpath $ORIGIN/lib)
...
cp -v "$BUILDDIR/bin/armsx2-sdl" "$STAGE/"
...
gather_from_depsdir "$STAGE/armsx2-sdl"
...
patchelf --set-rpath '$ORIGIN/lib' "$STAGE/armsx2-sdl"
