#!/bin/bash
#
# yaps2 fork: builds the macOS dependency set NATIVE arm64 (Apple Silicon) only.
# Derived from PCSX2's x86_64 build-dependencies.sh with three arch flips:
# CMAKE_OSX_ARCHITECTURES / APPLE_SILICON_PROCESSOR -> arm64, the ffmpeg
# cross-compile triple -> arm64 (+ --disable-x86asm), and MoltenVK VALID_ARCHS
# -> arm64. The old fat x86_64+arm64 "universal" variant was dropped — this fork
# ships arm64 only, so there is no reason to cross-build x86 slices.
...
XCODEBUILD_EXTRA_ARGS="VALID_ARCHS=arm64" ./fetchDependencies --macos
XCODEBUILD="set -o pipefail && xcodebuild VALID_ARCHS=arm64" make macos MVK_CONFIG_USE_METAL_ARGUMENT_BUFFERS=0 MVK_CONFIG_USE_METAL_PRIVATE_API=1
