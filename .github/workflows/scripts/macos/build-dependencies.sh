...
2b0d17a6e016f47b86f9c00e2cb82600041b1ea1f7d2a00c2d46ae542cbfed3c  plutovg-$PLUTOVG.tar.gz
49d5cfe772d3aa10cd4879f2f6e189f5083c08e4c8ea01bf3d5b87c97dfca7d2  plutosvg-$PLUTOSVG.tar.gz
e9efcdd17f86287748793cf21d106e461fcad8d103a3e5a23632afe93828660d  rapidyaml-$RAPIDYAML-src.tgz
4bf8cf2489d00848dcabbf2163204093776082da4217d5a5db45e4cbf335cedf  librashader-v$LIBRASHADER.tar.gz
...
if [ "$BUILD_LIBRASHADER" -ne 0 ] && ! command -v cargo >/dev/null 2>&1; then
...
if [ "$BUILD_LIBRASHADER" -ne 0 ]; then
	if ! command -v rustup >/dev/null 2>&1; then
		echo "rustup not found in PATH, skipping librashader build."
		BUILD_LIBRASHADER=0
	else
...
if [ "$BUILD_LIBRASHADER" -ne 0 ]; then
	...
	if [ -f "target/optimized/librashader.dylib" ]; then
		install -m 0755 "target/optimized/librashader.dylib" "$INSTALLDIR/lib/liblibrashader.dylib"
	elif [ -f "target/optimized/liblibrashader.dylib" ]; then
		install -m 0755 "target/optimized/liblibrashader.dylib" "$INSTALLDIR/lib/liblibrashader.dylib"
	elif [ -f "target/x86_64-apple-darwin/optimized/librashader.dylib" ]; then
		install -m 0755 "target/x86_64-apple-darwin/optimized/librashader.dylib" "$INSTALLDIR/lib/liblibrashader.dylib"
	elif [ -f "target/x86_64-apple-darwin/optimized/liblibrashader.dylib" ]; then
		install -m 0755 "target/x86_64-apple-darwin/optimized/liblibrashader.dylib" "$INSTALLDIR/lib/liblibrashader.dylib"
	else
