...
call :downloadfile "ffmpeg-no-nasm-fix-avu.patch" https://github.com/FFmpeg/FFmpeg/commit/59d75bf9e46938c694729dabf7db17646f1116ba.patch 45a66a8b018d70b2c7b10ab227da69586cb515fd1b1e8f5aa429cfd669e1e34c || goto error
call :downloadfile "ffmpeg-no-nasm-fix-swr.patch" https://github.com/FFmpeg/FFmpeg/commit/d5a47bf2b30afceff58a624dcc4142c18d70cbf9.patch 5f879bbb39aa4489cf8af29d0b99c54ebfadf0aaa47922f4a57c875ca73c1c25 || goto error
...
call :downloadfile "SVT-AV1-v%LIBSVTAV1%.zip" "https://gitlab.com/AOMediaCodec/SVT-AV1/-/archive/v%LIBSVTAV1%/SVT-AV1-v%LIBSVTAV1%.zip" 5a943b3c1c723b8d95a80dd4e2ebfac2bbd0513c9d79ca6936eafcebd8850a1e || goto error
...
if %BUILD_FFMPEG%==1 (
  ...
  cmake -DCMAKE_BUILD_TYPE=MinSizeRel -DCMAKE_PREFIX_PATH="%INSTALLDIR%" -DCMAKE_INSTALL_PREFIX="%INSTALLDIR%" -B build -G Ninja || goto error
  ninja -C build install || goto error
  cd ..
  ...
  if not !FOUND_NASM!==1 (
    ...
    rem The following somewhat commits fixes the issue
    rem ba94177242806ddf48e305ac52eee09a337a4a3b (conflicts)
    rem 59d75bf9e46938c694729dabf7db17646f1116ba
    rem d5a47bf2b30afceff58a624dcc4142c18d70cbf9
    %PATCH% -p1 < "..\ffmpeg-no-nasm-fix-avu.patch" || goto error
    %PATCH% -p1 < "..\ffmpeg-no-nasm-fix-swr.patch" || goto error
    rem We replace the conflicting patch with our own, and also patch swscale
  ...
  %BASH% configure --prefix="%INSTALLDIR%" --disable-all --disable-autodetect --disable-static --enable-shared --disable-debug --enable-small ^
    --toolchain=msvc --extra-cflags="-MD -GL" --extra-cxxflags="-MD -GL" --extra-ldflags="-LTCG" --extra-libs="advapi32.lib ole32.lib" !FFMPEG_NASM! --pkg-config="%INSTALLDIR%\bin\pkgconf.exe" ^
