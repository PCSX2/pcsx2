...
void extrapolate_aa1_triangle_edge(inout ProcessedVertex v0, ProcessedVertex v1, ProcessedVertex v2, mat2 dp_mat, vec2 dp)
{
	...
	{
		if (VS_FST != 0)
			dt = mat2(v1.ti.zw - v0.ti.zw, v2.ti.zw - v0.ti.zw);
		else
			dt = mat2(v1.t.xy - v0.t.xy, v2.t.xy - v0.t.xy);
	}

	// Get color delta if interpolating
	mat2x4 dc = mat2x4(0.0f);
	if (VS_IIP != 0)
		dc = mat2x4(v1.c - v0.c, v2.c - v0.c);
...
void main()
{
	...
	else if ((VS_EXPAND == VS_EXPAND_LINE) || (VS_EXPAND == VS_EXPAND_LINE_AA1))
	{
	...
	else if (VS_EXPAND == VS_EXPAND_SPRITE)
	{
	...
	else if (VS_EXPAND == VS_EXPAND_TRIANGLE_AA1)
	{
...
#define SW_BLEND uint(PS_BLEND_A != 0 || PS_BLEND_B != 0 || PS_BLEND_D != 0)
#define SW_BLEND_NEEDS_RT (SW_BLEND != 0 && (PS_BLEND_A == 1 || PS_BLEND_B == 1 || PS_BLEND_C == 1 || PS_BLEND_D == 1))
#define SW_AD_TO_HW uint(PS_BLEND_C == 1 && PS_A_MASKED != 0)
...
#if !UBER_SHADER
	#define PS_FEEDBACK_LOOP_IS_NEEDED_RT (PS_TEX_IS_FB == 1 || AFAIL_NEEDS_RT || PS_FBMASK || SW_BLEND_NEEDS_RT || SW_AD_TO_HW || (PS_DATE >= 5) || PS_CMASK)
	#define PS_FEEDBACK_LOOP_IS_NEEDED_DEPTH (AFAIL_NEEDS_DEPTH || ZTST_NEEDS_DEPTH || AA1_NEEDS_DEPTH || PS_ZMASK)
...
vec4 sample_c(vec2 uv)
{
	...
	else if (PS_REGION_RECT != 0)
		return texelFetch(Texture, ivec2(uv), 0);

	if (PS_ADJS == 0 && PS_ADJT == 0)
	{
		uv *= STScale;
	}
	else
...
vec4 clamp_wrap_uv(vec4 uv)
{
	...
	if (PS_WMS == PS_WMT)
	{
		...
		else if (PS_REGION_RECT == 1 && PS_WMS == 1)
		{
	...
	else
	{
		...
		else if (PS_REGION_RECT == 1 && PS_WMT == 1)
		{
...
uvec4 sample_4_index(vec4 uv)
{
	...
	uvec4 i;
	if (PS_RTA_SRC_CORRECTION != 0)
		i = uvec4(round(c * 128.25f));
	else
		i = uvec4(c * 255.5f);
...
vec4 sample_depth(vec2 st, ivec2 pos)
{
	...
	else if (PS_PAL_FMT != 0 && PS_TALES_OF_ABYSS_HLE == 0 && PS_URBAN_CHAOS_HLE == 0)
	{
...
vec4 sample_color(vec2 st)
{
	...
	if (PS_AEM_FMT == FMT_32 && PS_PAL_FMT == 0 && PS_RTA_SRC_CORRECTION != 0)
...
bool atst(vec4 C)
{
	...
	else if (PS_ATST == PS_ATST_GEQUAL)

		return (a >= AREF);

	else if (PS_ATST == PS_ATST_EQUAL)

		return (abs(a - AREF) <= 0.5f);

	else if (PS_ATST == PS_ATST_NOTEQUAL)
...
vec4 ps_color()
{
	...
	if (PS_FST == 0)
	{
		st = vsIn.t.xy / vsIn.t.w;
		st_int = vsIn.ti.zw / vsIn.t.w;
	}
	else
	{
		st = vsIn.ti.xy;
	...
	if (!NEEDS_TEX)
		T = vec4(0.0f);
	else if (PS_CHANNEL_FETCH == 1)
		T = fetch_red(ivec2(gl_FragCoord.xy + ChannelShuffleOffset));
	else if (PS_CHANNEL_FETCH == 2)
		T = fetch_green(ivec2(gl_FragCoord.xy + ChannelShuffleOffset));
	else if (PS_CHANNEL_FETCH == 3)
		T = fetch_blue(ivec2(gl_FragCoord.xy + ChannelShuffleOffset));
	else if (PS_CHANNEL_FETCH == 4)
		T = fetch_alpha(ivec2(gl_FragCoord.xy + ChannelShuffleOffset));
	else if (PS_CHANNEL_FETCH == 5)
		T = fetch_rgb(ivec2(gl_FragCoord.xy + ChannelShuffleOffset));
	else if (PS_CHANNEL_FETCH == 6)
		T = fetch_gXbY(ivec2(gl_FragCoord.xy + ChannelShuffleOffset));
	else if (PS_DEPTH_FMT > 0)
		T = sample_depth(st_int, ivec2(gl_FragCoord.xy));
	else
		T = sample_color(st);

	if (PS_SHUFFLE != 0 && PS_READ16_SRC == 0 && PS_SHUFFLE_SAME == 0 && !(PS_PROCESS_BA == SHUFFLE_READWRITE && PS_PROCESS_RG == SHUFFLE_READWRITE))
	{
...
void ps_fbmask(inout vec4 C)
{
	if (PS_FBMASK != 0)
	{
		vec4 RT = vec4(0.0f);
		if (PS_COLCLIP_HW == 1)
			RT = trunc(sample_from_rt() * 65535.0f);
		else
			RT = trunc(sample_from_rt() * 255.0f + 0.1f);
...
void ps_color_clamp_wrap(inout vec3 C)
{
	...
	if (SW_BLEND != 0 || (PS_DITHER > 0 && PS_DITHER < 3) || PS_FBMASK != 0)
	{

		if (PS_DST_FMT == FMT_16 && PS_BLEND_MIX == 0 && PS_ROUND_INV != 0)
...
void ps_blend(inout vec4 Color, inout vec4 As_rgba)
{
	...
	if (SW_BLEND != 0)
	{
		...
		if (PS_RTA_CORRECTION != 0)
			Ad = trunc(RT.a * 128.0f + 0.1f) / 128.0f;
		else
			Ad = trunc(RT.a * 255.0f + 0.1f) / 128.0f;
		...
		vec3 Cd = vec3(0.0f);
		if (PS_COLCLIP_HW == 1)
			Cd = trunc(RT.rgb * 65535.0f);
		else
			Cd = trunc(RT.rgb * 255.0f + 0.1f);
...
void main()
{
	...
	if (PS_DATE >= 5)
	{
		...
		if (PS_WRITE_RG == 1)
		{
			...
			rt_a = sample_from_rt().g;
		}
		else
		{
			rt_a = sample_from_rt().a;
		}
		...
		if ((PS_DATE & 3) == 1)
		{
			...
			if (PS_RTA_CORRECTION != 0)
				bad = (254.5f / 255.0f) < rt_a;
			else
				bad = (127.5f / 255.0f) < rt_a;
		}
		else if ((PS_DATE & 3) == 2)
		{
			// DATM == 1: Pixel with alpha equal to 0 will failed
			if (PS_RTA_CORRECTION != 0)
				bad = rt_a < (254.5f / 255.0f);
			else
				bad = rt_a < (127.5f / 255.0f);
		}
	...
	if (PS_AA1 != 0)
	{
		...
		if (PS_AA1 == PS_AA1_LINE)
		{
			...
			cov = clamp(LineCovScale * (1.0f - abs(vsIn.inv_cov)), 0.0f, 1.0f);
		}
		else
		{
			cov = clamp(1.0f - abs(vsIn.inv_cov), 0.0f, 1.0f);
		}
		if (PS_ABE != 0)
		{
	...
	if (SW_AD_TO_HW != 0)
	{
		vec4 RT = vec4(0.0f);
		if (PS_RTA_CORRECTION != 0)
			RT = trunc(sample_from_rt() * 128.0f + 0.1f);
		else
			RT = trunc(sample_from_rt() * 255.0f + 0.1f);
	...
	else if ((PS_DST_FMT == FMT_32) && (PS_FBA != 0))
	{
	...
	if (PS_SHUFFLE != 0)
	{
		if (PS_READ16_SRC == 0 && PS_SHUFFLE_SAME == 0 && !(PS_PROCESS_BA == SHUFFLE_READWRITE && PS_PROCESS_RG == SHUFFLE_READWRITE))
		{
	...
	// Output color scaling
		...
		else if (PS_AFAIL == AFAIL_RGB_ONLY || PS_AFAIL == AFAIL_RGB_ONLY_SW_Z)
		{
			if (!atst_pass)
			{
