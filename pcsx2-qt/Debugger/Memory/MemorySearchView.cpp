...
void MemorySearchView::onSearchButtonClicked()
{
	...
	if (!ok)
	{
		GuardedMessageBox::critical(this, tr("Debugger"), tr("Invalid start address"));
		return;
	}
	...
	if (!ok)
	{
		GuardedMessageBox::critical(this, tr("Debugger"), tr("Invalid end address"));
		return;
	}

	if (searchStart >= searchEnd)
	{
		GuardedMessageBox::critical(this, tr("Debugger"), tr("Start address can't be equal to or greater than the end address"));
		return;
	}
	...
	if (searchComparison != SearchComparison::UnknownValue)
	{
		if (doesSearchComparisonTakeInput(searchComparison))
		{
			...
			if (!ok)
			{
				GuardedMessageBox::critical(this, tr("Debugger"), tr("Invalid search value"));
				return;
			}
			...
			switch (searchType)
			{
				...
				case SearchType::ByteType:
					if (value <= std::numeric_limits<unsigned char>::max())
						break;
				default:
					GuardedMessageBox::critical(this, tr("Debugger"), tr("Value is larger than type"));
					return;
			}
		}
		...
		{
			GuardedMessageBox::critical(this, tr("Debugger"), tr("This search comparison can only be used with filter searches."));
			return;
		}
	}
