...
{
	DockSegmentedDropIndicatorOverlay::s_indicator_style =
		Host::GetStringSettingValue("Debugger/UserInterface", "DropIndicatorStyle", "Classic");
	if (DockSegmentedDropIndicatorOverlay::s_indicator_style == "Segmented" ||
		DockSegmentedDropIndicatorOverlay::s_indicator_style == "Minimalistic")
...
QString DockManager::currentLayoutName()
{
	if (m_current_layout == DockLayout::INVALID_INDEX)
		return QString();

	return m_layouts.at(m_current_layout).name();
}

bool DockManager::canResetCurrentLayout()
{
	if (m_current_layout == DockLayout::INVALID_INDEX)
		return false;

	return m_layouts.at(m_current_layout).canReset();
}

void DockManager::resetCurrentLayout()
{
	if (m_current_layout == DockLayout::INVALID_INDEX)
		return;

	DockLayout::Index current_layout = m_current_layout;

	switchToLayout(DockLayout::INVALID_INDEX);

	m_layouts.at(current_layout).reset();

	switchToLayout(current_layout);
}
