...
void DockManager::createWindowsMenu(QMenu* menu)
{
	...
	struct DebuggerWidgetToggleAction
	{
	...
	std::vector<DebuggerWidgetToggleAction> actions;
	...
	for (const auto& [unique_name, widget] : layout.debuggerWidgets())
	{
		...
		actions.emplace_back(widget->displayNameWithoutSuffix(), widget->displayNameSuffixNumber(), action);
	...
	for (const auto& [type, desc] : DockTables::DEBUGGER_WIDGETS)
	{
		if (!action_types.contains(type))
		{
			...
			actions.emplace_back(display_name, std::nullopt, action);
		}
	}

	std::sort(actions.begin(), actions.end(),
		[](const DebuggerWidgetToggleAction& lhs, const DebuggerWidgetToggleAction& rhs) {
	...
	for (const DebuggerWidgetToggleAction& action : actions)
