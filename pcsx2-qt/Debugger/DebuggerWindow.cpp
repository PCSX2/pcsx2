	...
	, m_dock_manager(new DockManager(this))
{
	...
	connect(m_ui.menuLayouts, &QMenu::aboutToShow, this, [this]() {
		m_ui.actionResetCurrentLayout->setEnabled(m_dock_manager->canResetCurrentLayout());
	});

	connect(m_ui.actionResetCurrentLayout, &QAction::triggered, this, [this]() {
		QString name = m_dock_manager->currentLayoutName();
		QMessageBox::StandardButton result = QMessageBox::question(
			g_debugger_window, tr("Confirmation"), tr("Are you sure you want to reset layout '%1'?").arg(name));

		if (result == QMessageBox::Yes)
			m_dock_manager->resetCurrentLayout();
	});
