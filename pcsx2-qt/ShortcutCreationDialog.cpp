...
void ShortcutCreationDialog::CreateShortcut(const std::string name, const std::string game_path, std::vector<std::string> passed_cli_args, std::string custom_args, bool is_desktop)
{
	...
	{
		Host::ReportErrorAsync(TRANSLATE_SV("ShortcutCreationDialog", "Failed to create shortcut"), TRANSLATE_SV("ShortcutCreationDialog", "A shortcut with the same name already exists."));
		return;
	}
