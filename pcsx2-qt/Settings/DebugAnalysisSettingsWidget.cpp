	...
	, m_dialog(dialog)
{
	...
	m_dialog->registerWidgetHelp(m_ui.demangleParameters, tr("Demangle Parameters"), tr("Checked"),
		tr("Include parameters lists in demangled function names."));
	...
	else
	{
		...
		m_ui.addSymbolFile->hide();
		m_ui.removeSymbolFile->hide();
	}
	...
	m_dialog->registerWidgetHelp(m_ui.functionScanMode, tr("Scan Mode"), tr("Checked"),
	...
	if (dialog->isPerGameSettings())
	{
		...
		SettingWidgetBinder::BindWidgetToStringSetting(sif, m_ui.addressRangeStart, "Debugger/Analysis", "FunctionScanStartAddress", "0");
		SettingWidgetBinder::BindWidgetToStringSetting(sif, m_ui.addressRangeEnd, "Debugger/Analysis", "FunctionScanEndAddress", "0");
	...
	m_dialog->registerWidgetHelp(m_ui.grayOutOverwrittenFunctions, tr("Gray Out Symbols For Overwritten Functions"), tr("Checked"),
		tr("Generate hashes for all the detected functions, and gray out the symbols displayed in the debugger for "
		   "functions that no longer match. Relocations being applied to a function will not change its hash."));
...
void DebugAnalysisSettingsWidget::setupSymbolSourceGrid()
{
	...
	if (!m_dialog || m_dialog->getSerial() == QtHost::GetCurrentGameSerial().toStdString())
	{
		...
		m_ui.startToModifySymbolSources->hide();
	}
	else
	{
...
void DebugAnalysisSettingsWidget::setupSymbolFileList()
{
	...
	connect(m_ui.addSymbolFile, &QPushButton::pressed, this, &DebugAnalysisSettingsWidget::addSymbolFile);
	connect(m_ui.removeSymbolFile, &QPushButton::pressed, this, &DebugAnalysisSettingsWidget::removeSymbolFile);
}
...
void DebugAnalysisSettingsWidget::updateEnabledStates()
{
	...
	m_ui.startToModifySymbolSources->setEnabled(!m_ui.automaticallyClearSymbols->isChecked());
