	...
	, m_dialog(dialog)
{
	...
	if (dialog->isPerGameSettings())
	{
		...
		m_dialog->registerWidgetHelp(m_ui.automaticallyClearSymbols, tr("Automatically Select Symbols To Clear"),
			tr("Automatically delete symbols that were generated by any previous analysis runs."), tr("Checked"));
	...
	m_dialog->registerWidgetHelp(m_ui.importFromElf, tr("Import From ELF"),
		tr("Import symbol tables stored in the game's boot ELF."), tr("Checked"));
	m_dialog->registerWidgetHelp(m_ui.importSymFileFromDefaultLocation, tr("Import Default .sym File"),
		tr("Import symbols from a .sym file with the same name as the loaded ISO file on disk if such a file exists."), tr("Checked"));
	m_dialog->registerWidgetHelp(m_ui.demangleSymbols, tr("Demangle Symbols"),
		tr("Demangle C++ symbols during the import process so that the function and global variable names shown in the "
		   "debugger are more readable."), tr("Checked"));
	m_dialog->registerWidgetHelp(m_ui.demangleParameters, tr("Demangle Parameters"),
		tr("Include parameter lists in demangled function names."), tr("Checked"));

	// Same as above. It only makes sense to load extra symbol files on a per-game basis.
	if (dialog->isPerGameSettings())
	{
		setupSymbolFileList();
	}
	...
	m_dialog->registerWidgetHelp(m_ui.functionScanMode, tr("Scan Mode"),
		tr("Choose where the function scanner looks to find functions. This option can be useful if the application "
		   "loads additional code at runtime."), tr("Scan ELF"));
	...
	if (dialog->isPerGameSettings())
	{
		...
		m_dialog->registerWidgetHelp(m_ui.customAddressRange, tr("Custom Address Range"),
			tr("Whether to look for functions from the address range specified (Checked), or from the ELF segment "
			   "containing the entry point (Unchecked)."), tr("Unchecked"));
	}
	else
	{
	...
	m_dialog->registerWidgetHelp(m_ui.grayOutOverwrittenFunctions, tr("Gray Out Symbols For Overwritten Functions"),
		tr("Generate hashes for all the detected functions, and gray out the symbols displayed in the debugger for "
		   "functions that no longer match."), tr("Checked"));
