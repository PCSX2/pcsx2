	...
	, m_dialog(dialog)
{
	...
	//:I18N COMMENT: The "User Preference" string will appear after the text "Recommended Value:"
	...
	dialog->registerWidgetHelp(m_ui.eeCycleSkipping, tr("Cycle Skip"), tr("Disabled"),
		tr("Makes the emulated Emotion Engine skip cycles. "
		   //:I18N COMMENT: SOTC = Shadow of the Colossus. A game's title, should not be translated unless an official translation exists.
	...
	dialog->registerWidgetHelp(m_ui.affinityControl, tr("Affinity Control"), tr("Disabled"),
		tr("Sets the priority for specific threads in a specific order ignoring the system scheduler. "
		   //:I18N COMMENT: P-Core = Performance Core, E-Core = Efficiency Core. See if Intel has official translations for these terms.
...
void EmulationSettingsWidget::initializeSpeedCombo(QComboBox* cb, const char* section, const char* key, float default_value)
{
	...
	//:I18N COMMENT: Every case that uses this particular string seems to refer to speeds: Normal Speed/Fast Forward Speed/Slow Motion Speed.
