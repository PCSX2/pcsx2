	...
	: SettingsWidget(settings_dialog, parent)
{
	...
	dialog()->registerWidgetHelp(m_gs.dumpGSData, tr("Dump GS Draws"), tr("Unchecked"),
		tr("Enables dumping of GS draws, textures, and render targets to disk for debugging."));
	dialog()->registerWidgetHelp(m_gs.saveRT, tr("Save Render Targets"), tr("Unchecked"),
		tr("Saves render target surfaces on each draw call when GS dumping is active."));
	dialog()->registerWidgetHelp(m_gs.saveFrame, tr("Save Frames"), tr("Unchecked"),
		tr("Saves completed frame buffer images when GS dumping is active."));
	dialog()->registerWidgetHelp(m_gs.saveTexture, tr("Save Textures"), tr("Unchecked"),
		tr("Saves source texture surfaces on each draw call when GS dumping is active."));
	dialog()->registerWidgetHelp(m_gs.saveDepth, tr("Save Depth Buffer"), tr("Unchecked"),
		tr("Saves depth buffer surfaces on each draw call when GS dumping is active."));
	dialog()->registerWidgetHelp(m_gs.saveAlpha, tr("Save Alpha Channel"), tr("Unchecked"),
		tr("Saves alpha channel data on each draw call when GS dumping is active."));
	dialog()->registerWidgetHelp(m_gs.saveInfo, tr("Save GS State Info"), tr("Unchecked"),
		tr("Saves text metadata describing the GS hardware register state on each draw call."));
	dialog()->registerWidgetHelp(m_gs.saveTransferImages, tr("Save Transfer Images"), tr("Unchecked"),
		tr("Saves host-to-local and local-to-host image transfer data when GS dumping is active."));
	dialog()->registerWidgetHelp(m_gs.saveDrawStats, tr("Save Draw Statistics"), tr("Unchecked"),
		tr("Saves performance metrics and statistics per draw call when GS dumping is active."));
	dialog()->registerWidgetHelp(m_gs.saveFrameStats, tr("Save Frame Statistics"), tr("Unchecked"),
	...
	dialog()->registerWidgetHelp(m_gs.saveDrawCount, tr("Save Draw Range Count"), tr("N/A"),
		tr("Total number of draw calls to dump per frame after the starting draw index."));
	dialog()->registerWidgetHelp(m_gs.saveFrameStart, tr("Save Frame Range Start"), tr("N/A"),
		tr("Starting frame number to begin dumping GS data."));
	dialog()->registerWidgetHelp(m_gs.saveFrameCount, tr("Save Frame Range Count"), tr("N/A"),
		tr("Total number of consecutive frames to dump before stopping."));
	dialog()->registerWidgetHelp(m_gs.hwDumpDirectory, tr("Hardware Dump Directory"), tr("Default"),
