	...
	, m_action_registry(std::move(action_registry))
{
	...
	connect(m_ui.restoreDefaultsButton, &QPushButton::clicked, this, &ToolbarCustomizationDialog::onRestoreDefaultsClicked);
...
void ToolbarCustomizationDialog::populateFromLayout(const QString& layout_str)
{
	...
	const QStringList items = layout_str.split(QLatin1Char(','), Qt::SkipEmptyParts);
	...
	for (const QString& item_id : items)
	{
		...
		if (trimmed == QStringLiteral("|") || trimmed == QStringLiteral("separator"))
		{
...
{
	QListWidgetItem* current = m_ui.availableList->currentItem();
	if (!current)
		return;

	const int row = m_ui.availableList->row(current);
	QListWidgetItem* taken = m_ui.availableList->takeItem(row);
