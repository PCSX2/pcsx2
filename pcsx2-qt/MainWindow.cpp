...
void MainWindow::rebuildToolbar()
{
	...
	const QStringList items = QString::fromStdString(layout_str).split(QLatin1Char(','), Qt::SkipEmptyParts);
	for (const QString& item_id : items)
	{
		const QString trimmed_id = item_id.trimmed();
		if (trimmed_id == QStringLiteral("|") || trimmed_id == QStringLiteral("separator"))
		{
			m_ui.toolBar->addSeparator();
		}
		else
		{
			auto it = action_map.find(trimmed_id);
			if (it != action_map.end() && it.value())
			{
				m_ui.toolBar->addAction(it.value());
			}
		}
	}
...
void MainWindow::onToolbarContextMenuRequested(const QPoint& pos)
{
	...
	QAction* restore_defaults = menu.addAction(tr("Restore Default Toolbar"));
	connect(restore_defaults, &QAction::triggered, this, [this]() {
