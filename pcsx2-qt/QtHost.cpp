...
void EmuThread::startVM(std::shared_ptr<VMBootParameters> boot_params)
{
	...
	const auto hardcore_disable_callback = [](std::string reason, VMBootRestartCallback restart_callback) {
	...
	const auto done_callback = [](VMBootResult result, const Error& error) {
