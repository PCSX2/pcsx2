...
EXPORT_C_(s32) FWinit()
{
	clc.SetName("FWnull");
	clc.InitLog();
	clc.Log.WriteLn("FWnull plugin version");
	clc.Log.WriteLn("Initializing FWnull");

	memset(phyregs, 0, sizeof(phyregs));
	fwregs = (s8 *)calloc(0x10000, 1);
	if (fwregs == NULL)
	{
		clc.Log.Message("Error allocating Memory");
		return -1;
	}
	return 0;
}
...
{
	clc.Log.WriteLn("Opening FWnull.");
...
{
	clc.Log.WriteLn("Closing FWnull.");
}

static void PHYWrite()
{
	u8 reg = (fwRu32(0x8414) >> 8) & 0xf;
	u8 data = fwRu32(0x8414) & 0xff;

	phyregs[reg] = data;

	fwRu32(0x8414) &= ~0x4000ffff;
}

static void PHYRead()
{
	u8 reg = (fwRu32(0x8414) >> 24) & 0xf;

	fwRu32(0x8414) &= ~0x80000000;

	fwRu32(0x8414) |= phyregs[reg] | (reg << 8);
...
EXPORT_C_(u32) FWread32(u32 addr)
{
	...
	clc.Log.WriteLn("FW read mem 0x%x: 0x%x", addr, ret);
...
EXPORT_C_(void) FWwrite32(u32 addr, u32 value)
{
	...
	case 0x1f808414:
		...
		// Im presuming we send that back to pcsx2 then. This register stores the result, plus whatever was written
		// (minus the read/write flag
	...
	clc.Log.WriteLn("FW write mem 0x%x: 0x%x", addr, value);
}
