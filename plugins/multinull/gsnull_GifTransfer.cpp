...
template <int index> void _GSgifTransfer(const u32 *pMem, u32 size)
{
	...
	while (size > 0)
	{
		// clcGS.Log.WriteLn(_T("Transfer(%08x, %d) START\n"), pMem, size);
		if (path->nloop == 0)
		{
			path->setTag(pMem);
			pMem += 4;
		...
		else
		{
			...
			{
				// clcGS.Log.WriteLn("%8.8x%8.8x %d L", ((u32*)&gs.regs)[1], *(u32*)&gs.regs, path->tag.nreg/4);
			...
			default: // GIF_IMAGE
				clcGS.Log.WriteLn("*** WARNING **** Unexpected GIFTag flag.");
				assert(0);
				path->nloop = 0;
				break;
			}
