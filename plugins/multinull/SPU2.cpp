/*  SPU2null
 *  Copyright (C) 2002-2015  SPU2null Team
 *
...
static u32 MemAddr[2];
static u32 g_nSpuInit = 0;
static u16 interrupt = 0;
static s8 *spu2regs = NULL;
static u16 *spu2mem = NULL;
static u16 *pSpuIrq[2] = {NULL};
static u32 dwEndChannel2[2] = {0}; // keeps track of what channels have ended
static u32 dwNoiseVal = 1;         // global noise generator

static s32 SPUCycles = 0, SPUWorkerCycles = 0;
static s32 SPUStartCycle[2];
static s32 SPUTargetCycle[2];

static int ADMAS4Write();
static int ADMAS7Write();

static void InitADSR();

static void (*irqCallbackSPU2)();     // func of main emu, called on spu irq
static void (*irqCallbackDMA4)() = 0; // func of main emu, called on spu irq
static void (*irqCallbackDMA7)() = 0; // func of main emu, called on spu irq

static const s32 f[5][2] = {{0, 0}, {60, 0}, {115, -52}, {98, -55}, {122, -60}};

static u32 RateTable[160];

// channels and voices
static VOICE_PROCESSED voices[SPU_NUMBER_VOICES + 1]; // +1 for modulation
...
{
	clc.SetName("spu2null");
	clc.InitLog();
	clc.Log.WriteLn("spu2null plugin version");
	clc.Log.WriteLn("Initializing spu2null");
...
EXPORT_C_(void) SPU2shutdown()
{
	...
	free(spu2mem);
	spu2mem = NULL;
	clc.Log.WriteLn("Shutting down spu2null.");
	clc.Log.Close();
}

// simulate SPU2 for 1ms
static void SPU2Worker();
...
static void InitADSR() // INIT ADSR
{
	u32 r, rs, rd;
	s32 i;
...
static int MixADSR(VOICE_PROCESSED *pvoice) // MIX ADSR
{
			...
			while (pChannel->spos >= 0x10000)
			{
				if (pChannel->iSBPos == 28) // 28 reached?
				{
					...
					if (flags & 1) // 1: stop/loop
					{
						...
						if (flags != 3 || pChannel->pLoop == NULL) // PETE: if we don't check exactly for 3, loop hang
...
EXPORT_C_(void) SPU2readDMA4Mem(u16 *pMem, int size)
{
	...
	clc.Log.WriteLn("SPU2 readDMA4Mem size %x, addr: %x\n", size, pMem);
...
EXPORT_C_(void) SPU2readDMA7Mem(u16 *pMem, int size)
{
	...
	clc.Log.WriteLn("SPU2 readDMA7Mem size %x, addr: %x\n", size, pMem);
...
EXPORT_C_(void) SPU2writeDMA4Mem(u16 *pMem, int size)
{
	...
	clc.Log.WriteLn("SPU2 writeDMA4Mem size %x, addr: %x\n", size, pMem);
...
EXPORT_C_(void) SPU2writeDMA7Mem(u16 *pMem, int size)
{
	...
	clc.Log.WriteLn("SPU2 writeDMA7Mem size %x, addr: %x\n", size, pMem);
...
{
	clc.Log.WriteLn("SPU2 interruptDMA4\n");
...
{
	clc.Log.WriteLn("SPU2 interruptDMA7\n");
...
// turn channels on
static void SoundOn(s32 start, s32 end, u16 val) // SOUND ON PSX COMAND
{
...
// turn channels off
static void SoundOff(s32 start, s32 end, u16 val) // SOUND OFF PSX COMMAND
{
	for (s32 ch = start; ch < end; ch++, val >>= 1) // loop channels
	{
...
static void FModOn(s32 start, s32 end, u16 val) // FMOD ON PSX COMMAND
{
	int ch;
...
EXPORT_C_(void) SPU2write(u32 mem, u16 value)
{
	...
	clc.Log.WriteLn("SPU2 write mem %x value %x\n", mem, value);
...
EXPORT_C_(u16) SPU2read(u32 mem)
{
	...
	clc.Log.WriteLn("SPU2 read mem %x: %x\n", mem, ret);
