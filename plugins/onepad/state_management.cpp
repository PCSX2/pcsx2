...
// FIXME
                ...
                if (pad->mode != MODE_DIGITAL) { // ANALOG || DS2 native
                    ...
                    query.response[5] = g_key_status.get([query.port][query.slot], PAD_R_RIGHT);
                    query.response[6] = g_key_status.get([query.port][query.slot], PAD_R_UP);
                    query.response[7] = g_key_status.get([query.port][query.slot], PAD_L_RIGHT);
                    query.response[8] = g_key_status.get([query.port][query.slot], PAD_L_UP);

                    if (pad->mode != MODE_ANALOG) { // DS2 native
                        query.numBytes = 21;

                        query.response[9] = !test_bit(buttons, 13) ? g_key_status.get([query.port][query.slot], PAD_RIGHT) : 0;
                        query.response[10] = !test_bit(buttons, 15) ? g_key_status.get([query.port][query.slot], PAD_LEFT) : 0;
                        query.response[11] = !test_bit(buttons, 12) ? g_key_status.get([query.port][query.slot], PAD_UP) : 0;
                        query.response[12] = !test_bit(buttons, 14) ? g_key_status.get([query.port][query.slot], PAD_DOWN) : 0;

                        query.response[13] = !test_bit(buttons, 4) ? g_key_status.get([query.port][query.slot], PAD_TRIANGLE) : 0;
                        query.response[14] = !test_bit(buttons, 5) ? g_key_status.get([query.port][query.slot], PAD_CIRCLE) : 0;
                        query.response[15] = !test_bit(buttons, 6) ? g_key_status.get([query.port][query.slot], PAD_CROSS) : 0;
                        query.response[16] = !test_bit(buttons, 7) ? g_key_status.get([query.port][query.slot], PAD_SQUARE) : 0;
                        query.response[17] = !test_bit(buttons, 2) ? g_key_status.get([query.port][query.slot], PAD_L1) : 0;
                        query.response[18] = !test_bit(buttons, 3) ? g_key_status.get([query.port][query.slot], PAD_R1) : 0;
                        query.response[19] = !test_bit(buttons, 0) ? g_key_status.get([query.port][query.slot], PAD_L2) : 0;
                        query.response[20] = !test_bit(buttons, 1) ? g_key_status.get([query.port][query.slot], PAD_R2) : 0;
                    }
                }
