...
namespace StringUtil
{
	...
	template <typename T> requires std::is_integral_v<T> std::optional<T> FromChars(const std::string_view str, int base = 10);
	template <typename T> requires std::is_integral_v<T> std::optional<T> FromChars(const std::string_view str, int base, std::string_view* endptr);
	template <typename T> requires std::is_floating_point_v<T> std::optional<T> FromChars(const std::string_view str);
	template <typename T> requires std::is_floating_point_v<T> std::optional<T> FromChars(const std::string_view str, std::string_view* endptr);

	/// Wrapper around `std::to_chars`
	template <typename T> requires std::is_integral_v<T> std::string ToChars(T value, int base = 10);
	template <typename T> requires std::is_floating_point_v<T> std::string ToChars(T value);

	/// Explicit override for booleans
	template <> std::optional<bool> FromChars(const std::string_view str, int base);
	template <>
