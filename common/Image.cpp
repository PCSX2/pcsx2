...
bool LoadUncompressedBMP(u32* pixels, const u8* src, const u8* data, u32 width, u32 height, const BMPInfoHeader& info_header, const std::vector<u32>& palette, bool flip_vertical)
{
			...
			switch (info_header.bit_count)
			{
				...
				case 16:
				{
					...
					if (info_header.compression == 3)
					{
						...
						u32 temp = r_mask; while (temp >>= 1) r_shift++;
						temp = g_mask; while (temp >>= 1) g_shift++;
						temp = b_mask; while (temp >>= 1) b_shift++;
...
bool LoadCompressedBMP(u32* pixels, const u8* src, u32 width, u32 height, const BMPInfoHeader& info_header, const std::vector<u32>& palette, bool flip_vertical)
{
	u32 src_pos = 0;
...
bool BMPBufferLoader(RGBA8Image* image, const void* buffer, size_t buffer_size)
{
	...
	{
		if (!LoadUncompressedBMP(pixels.data(), src, data, width, height, info_header, palette, flip_vertical))
		{
	...
	{
		if (!LoadCompressedBMP(pixels.data(), src, width, height, info_header, palette, flip_vertical))
		{
	...
	// Only V4/V5 headers explicitly support alpha, but we check pixel data for compatibility
	if (info_header.bit_count == 32)
	{
		// Check if any pixel has non zero alpha
		bool has_any_alpha = false;
		for (const u32 pixel : pixels)
		{
			if ((pixel >> 24) != 0)
			{
				has_any_alpha = true;
		...
		// If all alpha values are 0, the BMP likely doesn't use alpha
		// Set all pixels to fully opaque in that case
		if (!has_any_alpha)
		{
