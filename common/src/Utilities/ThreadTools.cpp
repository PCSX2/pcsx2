...
// to the pxThread table, nullptr is returned.  Since the main/ui thread is not created
// through pxThread it will also return nullptr.  Callers can use wxThread::IsMain() to
// test if the nullptr thread is the main thread.
...
{
    // Thread safety note: always detach the m_except pointer.  If we checked it for nullptr, the
...
{
    pxAssertDev(m_thread != nullptr, "nullptr thread object on ThreadError exception.");
    return *m_thread;
}
