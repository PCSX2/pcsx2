...
std::fstream FileUtils::fileStream(const fs::path& file_path, std::ios_base::openmode mode, bool create_if_nonexistant)
{
	...
	{
		std::ofstream temp_stream = fs::ofstream(file_path);
		temp_stream.close();
	}

	return fs::fstream(file_path, mode);
}
...
{
	return fs::ifstream(file_path, std::ios::binary);
}

std::ofstream FileUtils::binaryFileOutputStream(const fs::path& file_path)
{
	return fs::ofstream(file_path, std::ios::binary);
}
...
#else
	std::string curr_file_extension = file_path.extension().string();
...
#ifdef _WIN32
		backup_path.replace_extension(fmt::format("{}.{}", StringUtils::UTF8::narrow(file_path.extension().wstring()), "bak"));
#else
