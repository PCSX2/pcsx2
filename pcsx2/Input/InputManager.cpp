	...
	InputBindingInfo::Type binding_type, SettingsInterface& si, const char* section, const char* key)
{
	...
	if (migrate)
	{
		...
		{
			if (ibindings[i])
				new_bindings.push_back(ConvertInputBindingKeysToString(binding_type, ibindings[i]->keys, ibindings[i]->num_keys));
			else
...
void InputManager::AddPadBindings(SettingsInterface& si, u32 pad_index)
{
	...
	for (u32 macro_button_index = 0; macro_button_index < Pad::NUM_MACRO_BUTTONS_PER_CONTROLLER; macro_button_index++)
	{
		...
		if (!bindings.empty())
		{
			...
			AddBindings(
				bindings, InputAxisEventHandler{[pad_index, macro_button_index, deadzone](float value) {
		...
		switch (bi.bind_type)
		{
			...
			case InputBindingInfo::Type::HalfAxis:
			{
				...
				if (!bindings.empty())
				{
					...
					const float deadzone = si.GetFloatValue(section.c_str(), fmt::format("{}Deadzone", bi.name).c_str(), 0.0f);
					AddBindings(
						bindings, InputAxisEventHandler{[port, bind_index = bi.bind_index, sensitivity, deadzone](float value) {
