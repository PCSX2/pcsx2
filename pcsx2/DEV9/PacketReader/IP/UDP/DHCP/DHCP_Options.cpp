...
namespace PacketReader::IP::UDP::DHCP
{
	...
	DHCPopRouter::DHCPopRouter(std::vector<IP_Address> routerIPs)
	{
		routers = routerIPs;
	}
	...
	DHCPopDNS::DHCPopDNS(std::vector<IP_Address> dnsIPs)
	{
		dnsServers = dnsIPs;
	}
	...
	DHCPopHostName::DHCPopHostName(std::string name)
	{
		if (name.size() > 255)
		{
			Console.Error("DHCP Host Name too long");
	...
	DHCPopDnsName::DHCPopDnsName(std::string name)
	{
		if (name.size() > 255)
		{
			Console.Error("DHCP DNS Name too long");
	...
	DHCPopREQLIST::DHCPopREQLIST(std::vector<u8> requestList)
	{
	...
	DHCPopClassID::DHCPopClassID(std::string id)
	{
		if (id.size() > 255)
		{
			Console.Error("Class ID too long");
	...
	DHCPopClientID::DHCPopClientID(std::vector<u8> value)
	{
		clientID = value;
	}
