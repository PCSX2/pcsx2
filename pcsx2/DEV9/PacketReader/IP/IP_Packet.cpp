...
namespace PacketReader::IP
{
	IP_Packet::IP_Packet(u8* buffer, int bufferSize, bool fromICMP)
	{
		...
		NetLib::ReadByte08(buffer, &offset, &typeOfService); //TODO, Implement this
		...
		if (headerLength > 20) //IP options (if any)
		{
			...
			do
			{
				...
				switch (opKind)
				{
					...
					case 148:
					{
						IPopRouterAlert alert(buffer, offset);
						options.push_back(alert);
						break;
					}
					default:
					{
						Console.Error("Got Unknown IP Option %d with len %d", opKind, opLen);
						IPopUnk unk(buffer, offset);
						options.push_back(unk);
						break;
					}
				}
				offset += opLen;
		...
		payload = new IP_PayloadPtr(&buffer[offset], length - offset);
	}
	...
	void IP_Packet::SetPayload(IP_Payload* value, bool takeOwnership)
	{
		delete payload;
		payload = value;
	...
	void IP_Packet::WriteBytes(u8* buffer, int* offset)
	{
		...
		NetLib::WriteByte08(buffer, offset, typeOfService); //DSCP/ECN
	...
	void IP_Packet::CalculateChecksum()
	{
		...
		NetLib::WriteByte08(headerSegment, &counter, typeOfService); //DSCP/ECN
	...
	u16 IP_Packet::InternetChecksum(u8* buffer, int length)
	{
		...
		while (length > 1)
		{
			data = ((u32)(buffer[i]) << 8) | ((u32)(buffer[i + 1]) & 0xFF);
