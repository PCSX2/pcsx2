	...
	: sendThreadId{std::this_thread::get_id()}
{
	...
	IP_Address ps2IP, subnet, gateway;
	GetDHCPOverrides(&adapter, ps2IP, subnet, gateway);
...
#ifdef _WIN32
void SocketAdapter::GetDHCPOverrides(PIP_ADAPTER_ADDRESSES adapter, IP_Address& ps2IP, IP_Address& subnet, IP_Address& gateway)
#else
void SocketAdapter::GetDHCPOverrides(ifaddrs* adapter, IP_Address& ps2IP, IP_Address& subnet, IP_Address& gateway)
#endif
{
	if (EmuConfig.DEV9.LanMode)
	{
		...
		subnet = AdapterUtils::GetNetmask(adapter).value_or(IP_Address{{{255, 255, 255, 0}}});
