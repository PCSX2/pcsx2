...
void pgifInit(void) {
	...
	pgifDatRbC.buf = (void*)pgiffDatRbD;
...
void triggerPgifInt(int subCause) { //For the PGIF on EE side.
...
void getIrqCmd(u32 data) { //For the IOP GPU. This is triggered by the GP0(1Fh) - Interrupt Request (IRQ1) command.
...
u32 immRespHndl(u32 cmd, u32 data) { //Handles the GP1(10h) command, that requires immediate responce.
	...
	switch ((cmd & 0xFF)) {
		case 0x00:
		case 0x01: break; //Returns Nothing (old value in GPUREAD remains unchanged)
		case 0x02: data = pgif1reg & 0x001FFFFF; break; //Read Texture Window setting  ;GP0(E2h) ;20bit/MSBs=Nothing
		case 0x03: data = pgif2reg & 0x001FFFFF; break; //Read Draw area top left      ;GP0(E3h) ;20bit/MSBs=Nothing
		case 0x04: data = pgif3reg & 0x001FFFFF; break; //Read Draw area bottom right  ;GP0(E4h) ;20bit/MSBs=Nothing
		case 0x05: data = pgif4reg & 0x003FFFFF; break; //Read Draw offset             ;GP0(E5h) ;22bit
		case 0x06: break;		//Returns Nothing (old value in GPUREAD remains unchanged)
		case 0x07: data = 0x2; break;	//Read GPU Type (usually 2) See "GPU Versions" notes below
		case 0x08: data = 0; break;		//Unknown (Returns 00000000h)
...
void ckhCmdSetPgif(u32 cmd) { //Check GP1() command and configure PGIF accordingly.
...
u32 getUpdPgpuStatReg(void) {
	...
	switch ((PGpuStatReg >>29) & 0x3) {
		case 0x00: break; // When GP1(04h)=0 ---> Always zero (0)
...
int getDatRbC_Count() {
	if (pgifDatRbC.count >= 0x20) return 0x1F;
	else return pgifDatRbC.count;
}
...
void cmdRingBufGet(u32 *data) { //Used by PGIF/PS1DRV.
...
void drainPgpuDmaLl(void) {
	...
	if (pgifDatRbC.count >= ((pgifDatRbC.size) - PGIF_DAT_RB_LEAVE_FREE)) return; //buffer full - needs to be drained first.

	if ((nextAddr == 0x000C8000) || (nextAddr == 0x0) ||  (nextAddr == 0x3)) dntPrt++;
	//	if (dntPrt <5)
...
void drainPgpuDmaNrToGpu(void) {
	...
	//	Console.WriteLn( "PGPU NRM DMA data= %08X  addr %08X ", data, pgpuDmaTrAddr);
		...
		if ((crWordN % (pgpuDmaBcr & 0xFFFF)) == 0) //Because this is actually the "next word number", it can be compared to the size of the block. //End of block reached.
			if (pgpuDmaBcr >= 0x10000) pgpuDmaBcr = pgpuDmaBcr - 0x10000; //pgpuDmaBcr -= 0x10000;
...
u32 psxDma2GpuR(u32 addr) {
	...
	if (addr == PGPU_DMA_MADR) {
		...
		if ((PgpuDmaLlAct != 1) && (PgpuDmaNrActToGpu != 1) && (PgpuDmaNrActToIop !=1 )) pgpuDmaChcr &= ~0x01000000;
		data = pgpuDmaChcr;
...
//	if (mem == ) {
		...
		for (i=0; i<100; i++) stra[i] = iopMemRead8(data + i);
		stra[99] = 0;

		//Console.WriteLn("Wr to PS1 kernel file TTY descr. data = %08X  at addr %08X >%s< ", data, k, stra);
		...
		if ((strlen(strb) + strlen(stra)) >= 200) {
			...
			if ((stra[strlen(stra) - 1] == 0xA) || (stra[strlen(stra) - 1] == 0xD) || (stra[strlen(stra) - 2] == 0xA) || (stra[strlen(stra) - 2] == 0xD)) {
				Console.WriteLn(">%s<", strb);
				if (strcmp(strb, "VSync") == 0) flgint ^=1;
				if (strncmp(strb, "Execute", 5) == 0) { Console.WriteLn(" ###EXecute was just printed####");   doAnyIopLs =1;
				}
				strb[0] = 0;
			}
		}
...
void HPCoS_print(u32 mem, u32 data) {
	...
	if ((mem & 0x1FFFFFFF) == 0x1103A0) {
		...
		char str[200];
		//if (data == 0) {
			for (i=0; i<200; i++) { str[i] = iopMemRead8(0x117328 + i); if (str[i] == 0) break; }
			str[i] = 0;
