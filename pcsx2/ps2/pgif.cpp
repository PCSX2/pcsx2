...
u32 getUpdPgpuStatReg()
{
    ...
    if ((regs.pgif.ctrl & 0x10) || (regs.pgpu.stat & 0x20000000)) {  //Reverse direction bit GPU->CPU.
#else
...
void drainPgpuDmaLl()
{
    ...
    if (dma.llDma.currentWord >= dma.llDma.totalWords) {  //Reached end of sequence, or the beginning of a new one
...
void drainPgpuDmaNrToGpu()
{
    ...
    if (dma.normal.currentWord >= dma.normal.totalWords) {  //Reached end of sequence = complete
        dma.state.toGpuNormalActive = 0;
        pgpuDmaChcr &= ~DMA_TRIGGER;     //DMA is no longer active (in transfer)
        pgpuDmaChcr &= ~DMA_START_BUSY;  //Transfer completed => clear busy flag
        pgpuDmaIntr(1);
...
void drainPgpuDmaNrToIop()
{
    ...
    if (dma.normal.currentWord < dma.normal.totalWords) {  //This is not the best way, but... is there another?
        ringBufGet(&pgifDatRbC, &data);
