...
REG_END2
	...
	__forceinline int MinPossibleLOD() const
	{
		if (!IsMipMapActive())
			return 0;
		if (LCM)
			...
			return 0; // Unknown LOD so we must assume smallest is possible.
	}
	__forceinline int MaxPossibleLOD() const
	{
		if (!IsMipMapActive())
			return 0;
		if (LCM)
			...
			return static_cast<int>(MXL); // Unknown LOD so we must assume largest is possible.
	}
REG_END2
