...
void GSVertexTraceFMM::FindMinMax(const void* vertex, const u16* index, int count,
	...
	GSVector4i& pmin_out, GSVector4i& pmax_out)
{
	const int n = GSUtil::GetClassVertexCount(primclass); // FIXME: Make this constexpr
	...
	auto ProcessVertices = [&tmin, &tmax, &cmin, &cmax, &pmin, &pmax, n](
		const GSVertex& v0, const GSVertex& v1, const bool use_color) // FIXME: Make use_color template
	{
		// Process color
		if (color && use_color)
		{
	...
	if (iip)
	{
		// Gourad shading; treat all vertices the same.
		for (int i = 0; i < count - 1; i += 2) // 2x loop unroll
		{
			ProcessVertices(v[index[i]], v[index[i + 1]], true);
		}
		// Process last vertex if we had an odd number of vertices
		if (count & 1)
		{
			ProcessVertices(v[index[count - 1]], v[index[count - 1]], true);
		}
	}
	else if (primclass == GS_TRIANGLE_CLASS)
	{
		...
		if (count & 1)
		{
			ProcessVertices(v[index[count - 3]], v[index[count - 2]], false);
			ProcessVertices(v[index[count - 1]], v[index[count - 1]], true);
		}
	}
	else
