	...
	GSHWDrawConfig::ColorMaskSelector cms, ShaderConvert shader, bool linear)
{
	...
	pxAssert(HasVariableChannelMask(shader) || cms.wrgba == ShaderConvertWriteMask(shader));
	if (HasVariableChannelMask(shader))
...
void GSDevice::StretchRect(GSTexture* sTex, const GSVector4& sRect, GSTexture* dTex, const GSVector4& dRect, bool red, bool green, bool blue, bool alpha, ShaderConvert shader)
{
	...
	StretchRect(sTex, sRect, dTex, dRect, cms, shader, false);
}

void GSDevice::StretchRect(GSTexture* sTex, const GSVector4& sRect, GSTexture* dTex, const GSVector4& dRect,
	ShaderConvert shader, bool linear)
{
	StretchRect(sTex, sRect, dTex, dRect, GSHWDrawConfig::ColorMaskSelector(ShaderConvertWriteMask(shader)), shader, linear);
}

void GSDevice::StretchRect(GSTexture* sTex, GSTexture* dTex, const GSVector4& dRect,
	ShaderConvert shader, bool linear)
{
	...
	const MultiStretchRect* rects, u32 num_rects, GSTexture* dTex, ShaderConvert shader)
{
	for (u32 i = 0; i < num_rects; i++)
	{
		...
		pxAssert(HasVariableChannelMask(shader) || rects[0].wmask.wrgba == 0xf);
