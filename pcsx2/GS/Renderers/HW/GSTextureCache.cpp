	...
	for (auto i = map.begin(); i != map.end(); ++i)
	{
		...
		if (!s->m_target)
		{
			...
			if (((s->m_region.bits | fixed_tex0) != 0))
			{
				...
				{
					GSVector4i read_region = GSVector4i(0, 0, read_area.z, read_area.w);
					GSVector4i region_area = GSVector4i(s->m_region.GetMinX(), s->m_region.GetMinY(), s->m_region.HasX() ? s->m_region.GetMaxX() : (1 << s->m_TEX0.TW), s->m_region.HasY() ? s->m_region.GetMaxY() : (1 << s->m_TEX0.TW));
