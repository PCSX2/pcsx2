...
void GSTextureCache::Read(Target* t, const GSVector4i& r)
{
	...
	// yaps2 d84a571a: a 16-bit color readback at native scale doesn't need the GPU
	// format-convert pass — copy the RGBA8 target directly and pack to RGB5A1 on the CPU
	// below. The StretchRect pass otherwise sits inside the synchronous readback window
	// (submit -> fence wait), where a whole extra render pass costs far more than packing a
	// small rect on the CPU (readback-every-frame games pay it 3+ times/frame — OutRun 2006
	// SD865: 16.2->15.4ms). Upscaled targets keep the GPU path (which also downsamples).
	...
	// yaps2 2a5c0b1b: per-frame readback patterns (occlusion tests etc.) redraw the same
	// target before each read; let the backend schedule submissions around the next
	// producing draw (see GSDeviceVK::HintReadbackSource / the mid-frame kick).
	...
	switch (TEX0.PSM)
	{
		...
		case PSMZ16S:
			if (cpu_convert_rgb5a1)
			{
				// yaps2 d84a571a: bit-identical to ps_convert_rgb5a1_16bits — the shader
				// truncates each unorm channel back to its byte and packs
				// (r&0xF8)>>3 | (g&0xF8)<<2 | (b&0xF8)<<7 | (a&0x80)<<8; here c already
				// holds those bytes (direct RGBA8 copy of the native-scale target).
