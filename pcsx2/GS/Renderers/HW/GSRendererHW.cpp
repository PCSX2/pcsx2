...
void GSRendererHW::Draw()
{
	...
	if (m_process_texture)
	{
		...
		tmm = GetTextureMinMax(TEX0, MIP_CLAMP, m_vt.IsLinear(), true);
...
GSRendererHW::CLUTDrawTestResult GSRendererHW::PossibleCLUTDraw()
{
	...
	if (m_process_texture)
	{
		...
		const GSVector4i r = GetTextureMinMax(m_cached_ctx.TEX0, m_cached_ctx.CLAMP, m_vt.IsLinear(), true).coverage;
...
bool GSRendererHW::CanUseSwPrimRender(bool no_rt, bool no_ds, bool draw_sprite_tex)
{
	...
	if (PRIM->TME)
	{
		...
		if (src_target)
		{
			...
			{
				const GSVector4i tr(GetTextureMinMax(m_cached_ctx.TEX0, m_cached_ctx.CLAMP, m_vt.IsLinear(), true).coverage);
