...
void GSRendererHW::EmulateChannelShuffle(GSTexture** rt, const GSTextureCache::Source* tex, uint8& ps_channel, GSVector4i& ps_cb_channel)
{
	...
	if (m_channel_shuffle)
	{
		...
		else if ((tex->m_texture->GetType() == GSTexture::DepthStencil) && !(tex->m_32_bits_fmt))
		{
			...
			if ((m_context->FRAME.FBMSK & 0xFF0000) == 0xFF0000)
			{
				...
				ps_channel = ChannelFetch_TOA_HLE;
			}
			else
			{
				GL_INS("Urban Chaos Crazyness (Green extraction)");
				ps_channel = ChannelFetch_UC_HLE;
			}
		}
