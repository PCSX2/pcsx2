...
void GSRendererHW::EmulateAlphaTestSecondPass()
{
	...
	else
	{
		...
		{
			m_conf.alpha_second_pass.depth.zwe = 0;  // Disable Z write on second pass
		}
		...
		{
			m_conf.alpha_second_pass.depth.zwe = false; // Disable Z write on second pass
	...
	// If the alpha test prevent all write abort the draw.
	if (!(m_conf.colormask.wrgba || m_conf.depth.zwe))
