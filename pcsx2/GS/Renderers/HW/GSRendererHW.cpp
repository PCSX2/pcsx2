	...
	GSTextureCache::Source* tex, GSDevice::RecycledTexture& temp_tex)
{
			...
			g_gs_device->StartPipelineCompilationAsync(m_conf)))
	{
		...
		if (m_conf.ps.HasDepthROV() && !m_conf.ps.HasDepthROVWrite())
			m_conf.ps.zmask = true;
	}
}
...
void GSRendererHW::EmulateAlphaTestSecondPass()
{
		...
		m_conf.alpha_test == GSHWDrawConfig::AlphaTestMode::SIMPLE_RGB_ONLY)
	{
		...
		if (m_conf.alpha_second_pass.depth.zwe)
		{
			...
			m_conf.alpha_second_pass.ps.no_color1 = true; // Second pass shouldn't use dual source blend.
		}
