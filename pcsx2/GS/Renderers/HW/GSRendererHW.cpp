...
void GSRendererHW::GetZClampConfigVSPS(const HWCachedCtx& cached_ctx, const GSVertexTrace& vt, const bool force_enable, GSHWDrawConfig& config)
{
	const u32 max_z = 0xFFFFFFFF >> (GSLocalMemory::m_psm[cached_ctx.ZBUF.PSM].fmt * 8);
	const bool clamp_z = static_cast<u32>(GSVector4i(vt.m_max.p).z) > max_z;

	config.cb_vs.max_depth = GSVector2i(0xFFFFFFFF);
	config.ps.zclamp = 0;

	if (clamp_z || force_enable)
	{
		if (vt.m_primclass == GS_SPRITE_CLASS || vt.m_primclass == GS_POINT_CLASS)
		{
			config.cb_vs.max_depth = GSVector2i(max_z);
		}
		else if (!cached_ctx.ZBUF.ZMSK)
		{
	...
	if (GSConfig.HWAFAILFeedback || already_have_barriers || rgb_only_fbfetch)
	{
		...
		if (afail_needs_depth)
		{
			if (zwe)
			{
				...
				GetZClampConfigVSPS(cached_ctx, vt, true, config); // Z clamp is a proxy for Z write.
			}
