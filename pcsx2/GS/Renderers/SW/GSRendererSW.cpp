...
GSTexture* GSRendererSW::GetOutput(int i, int& y_offset)
{
	...
	if (m_dev->ResizeTexture(&m_texture[i], w, h))
	{
		...
		if (s_dump)
		{
			...
			{
				m_texture[i]->Save(m_dump_root + fmt::format("{:05d}_f{:d}_fr{:d}_{:05x}_{:s}.bmp", s_n, m_perfmon.GetFrame(), i, (int)DISPFB.Block(), psm_str(DISPFB.PSM)));
			}
		}
	}
	...
	if (s_dump)
	{
		...
		if (s_savet && s_n >= s_saven && PRIM->TME)
		{
			if (texture_shuffle)
			{
				...
				s = fmt::format("{:05d}_f{:d}_itexraw_{:05x}_32bits.bmp", s_n, frame, (int)m_context->TEX0.TBP0);
				m_mem.SaveBMP(m_dump_root + s, m_context->TEX0.TBP0, m_context->TEX0.TBW, 0, 1 << m_context->TEX0.TW, 1 << m_context->TEX0.TH);
			}

			s = fmt::format("{:05d}_f{:d}_itexraw_{:05x}_{:s}.bmp", s_n, frame, (int)m_context->TEX0.TBP0, psm_str(m_context->TEX0.PSM));
		...
		if (s_save && s_n >= s_saven)
		{
			...
			{
				// Dump the RT in 32 bits format. It helps to debug texture shuffle effect
				s = fmt::format("{:05d}_f{:d}_rt0_{:05x}_32bits.bmp", s_n, frame, m_context->FRAME.Block());
				m_mem.SaveBMP(m_dump_root + s, m_context->FRAME.Block(), m_context->FRAME.FBW, 0, GetFrameRect().width(), 512);
			}

			s = fmt::format("{:05d}_f{:d}_rt0_{:05x}_{:s}.bmp", s_n, frame, m_context->FRAME.Block(), psm_str(m_context->FRAME.PSM));
			m_mem.SaveBMP(m_dump_root + s, m_context->FRAME.Block(), m_context->FRAME.FBW, m_context->FRAME.PSM, GetFrameRect().width(), 512);
		}

		if (s_savez && s_n >= s_saven)
		{
			s = fmt::format("{:05d}_f{:d}_rz0_{:05x}_{:s}.bmp", s_n, frame, m_context->ZBUF.Block(), psm_str(m_context->ZBUF.PSM));
		...
		if (s_save && s_n >= s_saven)
		{
			...
			{
				// Dump the RT in 32 bits format. It helps to debug texture shuffle effect
				s = fmt::format("{:05d}_f%{:d}_rt1_{:05x}_32bits.bmp", s_n, frame, m_context->FRAME.Block());
				m_mem.SaveBMP(m_dump_root + s, m_context->FRAME.Block(), m_context->FRAME.FBW, 0, GetFrameRect().width(), 512);
			}

			s = fmt::format("{:05d}_f{:d}_rt1_{:05x}_{:s}.bmp", s_n, frame, m_context->FRAME.Block(), psm_str(m_context->FRAME.PSM));
			m_mem.SaveBMP(m_dump_root + s, m_context->FRAME.Block(), m_context->FRAME.FBW, m_context->FRAME.PSM, GetFrameRect().width(), 512);
		}

		if (s_savez && s_n >= s_saven)
		{
			s = fmt::format("{:05d}_f{:d}_rz1_{:05x}_{:s}.bmp", s_n, frame, m_context->ZBUF.Block(), psm_str(m_context->ZBUF.PSM));
...
void GSRendererSW::Sync(int reason)
{
	...
	if (0) if (LOG)
	{
		...
		if (s_save)
		{
			s = fmt::format("{:05d}_f{:d}_rt1_{:05x}_{:s}.bmp", s_n, m_perfmon.GetFrame(), m_context->FRAME.Block(), psm_str(m_context->FRAME.PSM));
		...
		if (s_savez)
		{
			s = fmt::format("{:05d}_f{:d}_zb1_{:05x}_{:s}.bmp", s_n, m_perfmon.GetFrame(), m_context->ZBUF.Block(), psm_str(m_context->ZBUF.PSM));
	...
	if (m_parent->s_dump)
	{
		...
		if (m_parent->s_savet && m_parent->s_n >= m_parent->s_saven)
		{
			for (size_t i = 0; m_tex[i].t != NULL; i++)
			{
				...
				s = fmt::format("{:05d}_f{:d}_itex{:d}_{:05x}_{:s}.bmp", m_parent->s_n, frame, i, TEX0.TBP0, psm_str(TEX0.PSM));
			...
			if (global.clut != NULL)
			{
				...
				s = fmt::format("{:05d}_f{:d}_itexp_{:05x}_{:s}.bmp", m_parent->s_n, frame, (int)m_parent->m_context->TEX0.CBP, psm_str(m_parent->m_context->TEX0.CPSM));
