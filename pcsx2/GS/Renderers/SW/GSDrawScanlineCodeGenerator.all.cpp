...
using GSScanlineConstantDataT = GSScanlineConstantData256B;
#else
static constexpr const GSScanlineConstantData128B& g_const = g_const_128b;
using GSScanlineConstantDataT = GSScanlineConstantData128B;
#endif
	...
	const XYm& tmp2, const XYm& tmp3, const XYm& tmp4, const XYm& tmp5, const XYm& tmp6)
{
	for (int i = 0; i < 2; i++)
	{
		...
		psubd(tmp2, tmp1);
		pabsd(tmp2, tmp2);
		movaps(tmp5, ptr[_m_const + offsetof(GSScanlineConstantDataT, m_round_threshold)]);
		pcmpgtd(tmp2, tmp5);
		pcmpeqd(tmp6, tmp6);
		pxor(tmp2, tmp6);
