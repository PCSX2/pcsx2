	...
	const GSVector4i* scissor, ID3D11DepthStencilView* read_only_dsv)
{
	...
	const bool changed = (m_state.rtv != rtv || m_state.dsrtv != dsrtv || m_state.dsv != dsv || m_state.rt_uav != rt_uav || m_state.ds_uav != ds_uav);
...
void GSDevice11::RenderHW(GSHWDrawConfig& config)
{
	...
	if (!(draw_rt || draw_rt_rov) && draw_ds && m_state.current_rt && config.tex != m_state.current_rt && m_state.current_rt->GetSize() == draw_ds->GetSize() && !draw_ds_as_rt)
	{
		draw_rt = m_state.current_rt;
	}
	else if (!(draw_ds || draw_ds_rov) && draw_rt && m_state.current_ds && config.tex != m_state.current_ds && m_state.current_ds->GetSize() == draw_rt->GetSize() && !draw_ds_as_rt)
	{
