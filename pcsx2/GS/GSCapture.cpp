...
bool GSCapture::ProcessAudioPackets(s64 video_pts)
{
																...
																s_next_audio_pts, s_audio_codec_context->time_base) > 0))
	{
		...
		// Apply volume multiplier if not 100%
		const int volume = GSConfig.AudioCaptureVolume;
		const float volume_mult = (volume != 100) ? (static_cast<float>(volume) / 100.0f) : 1.0f;
		...
		if (!s_swr_context)
		{
			...
			if (s_audio_frame_planar)
			{
				...
				for (u32 i = 0; i < AUDIO_CHANNELS; i++)
				{
					...
					for (u32 j = 0; j < this_batch; j++)
					{
						...
						if (volume_mult != 1.0f)
						{
							float sample = static_cast<float>(src) * volume_mult;
							if (sample > 32767.0f)
								sample = 32767.0f;
							else if (sample < -32768.0f)
								sample = -32768.0f;
							const s16 scaled_sample = static_cast<s16>(sample);
							std::memcpy(output, &scaled_sample, sizeof(s16));
						}
						else
						{
							std::memcpy(output, &src, sizeof(s16));
						}
			...
			else
			{
				...
				if (volume_mult != 1.0f)
				{
					...
					{
						const float sample = static_cast<float>(input[i]) * volume_mult;
						output[i] = static_cast<s16>(std::clamp(sample, -32768.0f, 32767.0f));
					}
				}
				else
				{
		...
		else
		{
			...
			if (volume_mult != 1.0f)
			{
				...
				for (u32 i = 0; i < this_batch * AUDIO_CHANNELS; i++)
				{
					const float sample = static_cast<float>(input_s16[i]) * volume_mult;
					volume_buffer[i] = static_cast<s16>(std::clamp(sample, -32768.0f, 32767.0f));
				}
