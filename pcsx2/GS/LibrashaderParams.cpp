...
namespace
{
	...
	bool IsLayoutParam(std::string_view name)
	{
	...
	void CollectPasses(
		const std::string& current_path, std::map<int, std::string>& by_index, std::unordered_set<std::string>& visited)
	{
		const std::string native_path = Path::ToNativePath(StringUtil::StripWhitespace(current_path));
		if (native_path.empty() || !FileSystem::FileExists(native_path.c_str()))
			return;

		if (!visited.insert(native_path).second)
			return;
		...
		const std::string dir = std::string(Path::GetDirectory(native_path));
		std::vector<std::string> references;
		std::map<int, std::string> current_shaders;
		...
		while (line_start < contents->size())
		{
			...
			{
				std::string ref = TrimQuotes(line.substr(std::strlen("#reference")));
				if (!ref.empty())
			...
			else if (line.starts_with("reference"))
			{
				...
				{
					std::string ref = TrimQuotes(line.substr(eq_pos + 1));
					if (!ref.empty())
						references.push_back(std::move(ref));
				}
			}
				...
				if (shader_pos != std::string_view::npos)
				{
					...
					if (index_end > index_start && eq_pos != std::string_view::npos)
					{
						...
						if (!value.empty() && value[0] != '#')
							current_shaders[idx] = std::move(value);
					}
				}
			}
		...
		{
			std::string ref_full_path = Path::IsAbsolute(ref) ? ref : Path::Combine(dir, ref);
			CollectPasses(ref_full_path, by_index, visited);
		}

		for (auto& [idx, path] : current_shaders)
			by_index[idx] = std::move(path);
	}
} // namespace
...
std::vector<std::string> GetSlangpPassNames(const std::string& preset_path)
{
	...
	CollectPasses(native_path, by_index, visited);
...
bool LibrashaderParam::IsSpacer() const
{
	...
	const bool empty_desc = StringUtil::StripWhitespace(description).empty();
	if (empty_desc && (name.ends_with("_nonono") || name.ends_with("_LINE") || name.starts_with("dummy") || name.starts_with("bogus")))
		return true;
