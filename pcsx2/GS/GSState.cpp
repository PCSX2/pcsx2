...
{
	for (u32 i = 1; i < m_used_buffers_idx; i++)
	{
		if (m_index_buffers[i].tail)
		...
		{
			memcpy(m_vertex_buffers[0].buff, m_vertex_buffers[i].buff, sizeof(GSVertex) * m_vertex_buffers[i].tail);
			
			m_vertex_buffers[0].head = m_vertex_buffers[i].head;
			m_vertex_buffers[0].tail = m_vertex_buffers[i].tail;
			m_vertex_buffers[0].next = m_vertex_buffers[i].next;

			if (m_vertex_buffers[0].tail != 0)
			{
				memcpy(m_vertex_buffers[0].xy, m_vertex_buffers[i].xy, sizeof(m_vertex_buffers[i].xy));
				m_vertex_buffers[0].xyhead = m_vertex_buffers[i].xyhead;
				m_vertex_buffers[0].xy_tail = m_vertex_buffers[i].xy_tail;
			}
			else
			{
				m_vertex_buffers[0].xy_tail = 0;
			}

			memcpy(&m_env_buffers[0], &m_env_buffers[i], sizeof(m_env_buffers[i]));
		}
	...
	m_env_buffers[0].draw_rect = GSVector4i::zero();
	m_index = &m_index_buffers[0];
	m_vertex = &m_vertex_buffers[0];
...
void GSState::FlushBuffers(bool use_flush_reason, GSFlushReason flush_reason)
{
	...
	if (m_used_buffers_idx > 0)
	{
		...
		for (u32 i = 0; i < m_used_buffers_idx; i++)
		{
			...
			if (use_flush_reason && i == current_idx)
...
void GSState::PushBuffer()
{
	...
	if (m_used_buffers_idx < MAX_DRAW_BUFFERS)
	{
		...
		m_env_buffers[m_current_buffer_idx].draw_rect = m_dirty_gs_regs;
		temp_draw_rect = m_env_buffers[m_current_buffer_idx].draw_rect;
		m_dirty_gs_regs = 0;
		m_used_buffers_idx++;
...
bool GSState::CanBufferNewDraw()
{
	...
	if (base_context.ZBUF.ZMSK || cur_context.FRAME.FBP != base_context.FRAME.FBP || cur_context.ZBUF.ZBP != base_context.ZBUF.ZBP)
	{
		//DevCon.Warning("Flushing, cannot buffer draw due to incompatible base");
		return false;
	}

	if (m_env.PRIM.PRIM != m_env_buffers[0].m_env.PRIM.PRIM || GSUtil::GetPrimClass(m_env.PRIM.PRIM) != GS_TRIANGLE_CLASS)
		return false;

	// It's expected that the Z used will match what has currently been drawn, so GREATER or ALWAYS aren't realy usable.
	if (!cur_context.TEST.ZTE || cur_context.TEST.ZTST != ZTST_GEQUAL)
		return false;
	...
	for (u32 i = 0; i < m_used_buffers_idx; i++)
	{
		...
		if (m_env.PRIM != m_env_buffers[i].m_env.PRIM)
			continue;
		...
		if (!std::memcmp(&m_env_buffers[i].m_env, &m_env, 88))
		{
			...
			if (i != m_current_buffer_idx)
			{
				...
				const u32 base = /*std::min(*/m_vertex_buffers[m_current_buffer_idx].head/*, m_vertex_buffers[m_current_buffer_idx].next)*/;
				const u32 copy_amt = m_vertex_buffers[m_current_buffer_idx].tail - base;

				m_recent_buffer_switch = true;
				//i < m_current_buffer_idx;
				m_vertex->tail = m_index->buff[m_index->tail - 1] + 1;

				if (copy_amt)
					memcpy(&m_vertex->buff[m_vertex->tail], &m_vertex_buffers[m_current_buffer_idx].buff[base], sizeof(GSVertex) * copy_amt);

				m_vertex->head = m_vertex->tail + (m_vertex_buffers[m_current_buffer_idx].head - base);
				m_vertex->next = m_vertex->head;
				//m_vertex->tail + (m_vertex_buffers[m_current_buffer_idx].next - base);
				m_vertex->tail += copy_amt;
	...
	if (m_used_buffers_idx >= MAX_DRAW_BUFFERS)
		return false;
	/*if (!cur_context.ZBUF.ZMSK || (cur_context.TEST.ATE && cur_context.TEST.AFAIL == AFAIL_ZB_ONLY))
		return false;*/
	/*if (!cur_context.ZBUF.ZMSK  && (!m_env.PRIM.ABE || cur_context.ALPHA.IsOpaque() || !cur_context.ALPHA.IsCdInBlend()))
		return false;
		*/
	PushBuffer();
...
void GSState::ApplyTEX0(GIFRegTEX0& TEX0)
{
	...
	if (wt)
	{
		for (u32 b = 0; b < m_used_buffers_idx; b++)
		{
	...
	for (u32 i = 0; i < m_used_buffers_idx; i++)
	{
		...
		if (cur_index_buff->tail > 0)
		{
			...
			if (frame_required)
			{
				...
				if ((req_write && (blit.DBP == prev_ctx.FRAME.Block() || GSLocalMemory::HasOverlap(blit.DBP, blit.DBW, blit.DPSM, write_rect, prev_ctx.FRAME.Block(), prev_ctx.FRAME.FBW, prev_ctx.FRAME.PSM, m_env_buffers[i].draw_rect))) ||
					(req_read && (blit.DBP == prev_ctx.FRAME.Block() || GSLocalMemory::HasOverlap(blit.SBP, blit.SBW, blit.SPSM, read_rect, prev_ctx.FRAME.Block(), prev_ctx.FRAME.FBW, prev_ctx.FRAME.PSM, m_env_buffers[i].draw_rect))))
				{
					Flush(reason);
					continue;
				}
			...
			if (zbuf_required)
			{
				...
				if ((req_write && (blit.DBP == prev_ctx.ZBUF.Block() || GSLocalMemory::HasOverlap(blit.DBP, blit.DBW, blit.DPSM, write_rect, prev_ctx.ZBUF.Block(), prev_ctx.FRAME.FBW, prev_ctx.ZBUF.PSM, m_env_buffers[i].draw_rect))) ||
					(req_read && (blit.DBP == prev_ctx.ZBUF.Block() || GSLocalMemory::HasOverlap(blit.SBP, blit.SBW, blit.SPSM, read_rect, prev_ctx.ZBUF.Block(), prev_ctx.FRAME.FBW, prev_ctx.ZBUF.PSM, m_env_buffers[i].draw_rect))))
				{
					Flush(reason);
					continue;
				}
...
bool GSState::CheckOverlapVerts(u32 n)
{
	if (m_recent_buffer_switch && ((m_vertex->tail + 1) - m_vertex->head) == n)
	{
		...
		/*if (!m_env.PRIM.ABE || m_env.CTXT[m_env.PRIM.CTXT].ALPHA.IsOpaque())
			return false;*/
		...
		if (m_used_buffers_idx > 1)
		{
			...
			for (u32 i = 0; i < (n - 1); i++)
			{
				...
				if (m_env.PRIM.PRIM == GS_TRIANGLEFAN)
					pre_vert = GSVector2i(m_vertex->xyhead.x - m_context->XYOFFSET.OFX, m_vertex->xyhead.y - m_context->XYOFFSET.OFY);
				else
					pre_vert = GSVector2i(v[pos].XYZ.X - m_context->XYOFFSET.OFX, v[pos].XYZ.Y - m_context->XYOFFSET.OFY);
			...
			//GSVector4i new_area_copy = new_area;

			/*if (!new_area.rintersect(temp_draw_rect).eq(new_area))
				return true;*/

			/*if (!new_area.rintersect(temp_draw_rect).rempty())
				check = true;*/
				/*new_area_copy = new_area_copy.runion(temp_draw_rect);
			else
				new_area_copy = temp_draw_rect;*/

			for (u32 b = m_current_buffer_idx; b < m_used_buffers_idx; b++)
			{
				/*if ((!m_env_buffers[b].m_env.PRIM.ABE || !m_env_buffers[b].m_env.CTXT[m_env_buffers[b].m_env.PRIM.CTXT].ALPHA.IsOpaque()) && !temp_draw_rect.eq(m_env_buffers[b].draw_rect))
				{
					if (!((!m_env_buffers[b].m_env.PRIM.ABE || !m_env_buffers[b].m_env.CTXT[m_env_buffers[b].m_env.PRIM.CTXT].ALPHA.IsOpaque()) && !temp_draw_rect.eq(m_env_buffers[b].draw_rect) && !new_area.rintersect(m_env_buffers[b].draw_rect).rempty()))
						DevCon.Warning("poo?");
					return true;
				}*/
				if (new_area.rintersect(m_env_buffers[b].draw_rect).eq(new_area)/* && !temp_draw_rect.rintersect(m_env_buffers[b].draw_rect).rempty()*/)
					return true;
			}
