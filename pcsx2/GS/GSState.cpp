...
__forceinline void GSState::HandleAutoFlush()
{
	...
	if (IsAutoFlushDraw(prim, tex_layer))
	{
		...
		xy_coord.x = (m_v.XYZ.X - m_context->XYOFFSET.OFX) >> 4;
		xy_coord.y = (m_v.XYZ.Y - m_context->XYOFFSET.OFY) >> 4;
		if (PRIM->FST)
		{
		...
		for (int i = 0; i < (n - 1); i++)
		{
			...
			xy_coord.x = (v->XYZ.X - m_context->XYOFFSET.OFX) >> 4;
			xy_coord.y = (v->XYZ.Y - m_context->XYOFFSET.OFY) >> 4;
			if (PRIM->FST)
			{
