...
// If expand_region_mode is true, the rectangle can be outside the bounds of TW/TH.
// This actually happens in games when the texture buffer is big enough the it's legal to
// sample outside the bounds of TW/TH. If false, the rectangle is always within the bounds of TW/TH.
	...
	GIFRegTEX0 TEX0, u32 fst, bool linear, const GSVertex* vertex, u16 index0, u16 index1, GSVector4* minmax)
{
	...
	float u0_interp = ((x1 - std::ceil(x0)) * u0 + (std::ceil(x0) - x0) * u1) / (x1 - x0);
	float v0_interp = ((y1 - std::ceil(y0)) * v0 + (std::ceil(y0) - y0) * v1) / (y1 - y0);
	float u1_interp = ((x1 - std::floor(x1)) * u0 + (std::floor(x1) - x0) * u1) / (x1 - x0);
	float v1_interp = ((y1 - std::floor(y1)) * v0 + (std::floor(y1) - y0) * v1) / (y1 - y0);
