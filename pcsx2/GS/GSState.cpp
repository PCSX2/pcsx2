...
void GSState::ResetDrawBufferIdx()
{
	for (u32 i = 1; i < m_used_buffers_idx; i++)
	{
		...
		{
			const u32 copy_amt = m_vertex_buffers[i].tail - m_vertex_buffers[i].head;
			memcpy(m_vertex_buffers[0].buff, &m_vertex_buffers[i].buff[m_vertex_buffers[i].head], sizeof(GSVertex) * copy_amt);
			m_vertex_buffers[0].head = m_vertex_buffers[i].head;
			m_vertex_buffers[0].tail = m_vertex_buffers[i].tail;
			m_vertex_buffers[0].next = m_vertex_buffers[i].next > m_vertex_buffers[i].head ? m_vertex_buffers[i].next - m_vertex_buffers[i].head : 0;

			if (m_index_buffers[i].tail == 0)
				m_vertex_buffers[0].next = 0;

			for (u32 i = 0; i < copy_amt; i++)
			{
				GSVector4i* RESTRICT vert_ptr = (GSVector4i*)&m_vertex_buffers[0].buff[i];
				GSVector4i v = vert_ptr[1];
				v = v.xxxx().u16to32().sub32(m_xyof);
				v = v.blend32<12>(v.sra32<4>());
				m_vertex_buffers[0].xy[i & 3] = v;
				m_vertex_buffers[0].xy_tail = copy_amt;
			}
			memcpy(&m_env_buffers[0], &m_env_buffers[i], sizeof(m_env_buffers[i]));
		}

		m_index_buffers[i].tail = 0;
		memset(&m_env_buffers[i].m_env, 0, sizeof(GSDrawingEnvironment));
	...
	m_backed_up_ctx = m_env_buffers[0].m_backed_up_ctx;
	m_dirty_gs_regs = m_env_buffers[0].m_dirty_regs;
...
{
	const u32 backup_dirty = m_dirty_gs_regs;
	...
	if (m_used_buffers_idx > 0)
	{
		...
		//DevCon.Warning("Flushing %d draw buffers", m_used_buffers_idx);
	...
	m_dirty_gs_regs = backup_dirty; // Restore dirtying reasion.
...
void GSState::PushBuffer()
{
	...
	if (m_used_buffers_idx < MAX_DRAW_BUFFERS)
	{
		...
		const u32 next_off = m_vertex_buffers[m_current_buffer_idx].next > m_vertex_buffers[m_current_buffer_idx].head ? m_vertex_buffers[m_current_buffer_idx].next - m_vertex_buffers[m_current_buffer_idx].head : 0;
		if (copy_amt)
			memcpy(m_vertex->buff, &m_vertex_buffers[m_current_buffer_idx].buff[m_vertex_buffers[m_current_buffer_idx].head], sizeof(GSVertex) * copy_amt);

		m_vertex->head = 0;
		m_vertex->tail = copy_amt;
		m_vertex->next = next_off;

		if (m_vertex_buffers[m_current_buffer_idx].tail == 0)
			m_vertex->next = 0;
		...
		DevCon.Warning("WARNING: Failed to pick buffer draw %d", s_n);
}
...
bool GSState::CanBufferNewDraw()
{
	...
	if ((base_context.ZBUF.ZMSK && !cur_context.ZBUF.ZMSK) || cur_context.FRAME.FBP != base_context.FRAME.FBP || cur_context.ZBUF.ZBP != base_context.ZBUF.ZBP)
	{
		//DevCon.Warning("Flushing, cannot buffer draw due to incompatible base");
		return false;
	}

	if (cur_context.TEST.ZTE && cur_context.TEST.ZTST > ZTST_ALWAYS)
	{
		...
		for (u32 i = 0; i < m_used_buffers_idx; i++)
		{
			...
			if (!std::memcmp(&m_env_buffers[i].m_env, &m_env, 88))
			{
				...
				if (m_env.PRIM.TME)
				{
					...
					const u32 clamp_mask = 0xF | (buffered_ctx.CTXT[ctx].CLAMP.WMS > 1 ? (0xFFFFF << 4) : 0) | (buffered_ctx.CTXT[ctx].CLAMP.WMT > 1 ? (0xFFFFF << 24) : 0); 
					if ((buffered_ctx.CTXT[ctx].CLAMP.U32[0] ^ cur_context.CLAMP.U32[0]) & clamp_mask)
						continue;
					if (GSLocalMemory::m_psm[cur_context.TEX0.PSM].trbpp != 32 && buffered_ctx.TEXA.U64 ^ m_env.TEXA.U64)
						continue;
					if (buffered_ctx.CTXT[ctx].TEX1.U32[0] ^ cur_context.TEX1.U32[0] || (cur_context.TEX1.MXL && buffered_ctx.CTXT[ctx].TEX1.U32[0] ^ cur_context.TEX1.U32[0]))
						continue;
				}
				if (buffered_ctx.CTXT[ctx].ALPHA.U32[0] ^ cur_context.ALPHA.U32[0])
					continue;
				if (buffered_ctx.CTXT[ctx].FBA != cur_context.FBA)
					continue;
				if (buffered_ctx.CTXT[ctx].TEST.U64 ^ cur_context.TEST.U64)
					continue;
				...
				if (i != m_current_buffer_idx)
				{
					...
					m_vertex->tail = m_vertex->head + copy_amt;
					m_vertex->next = m_vertex->head + next_off;

					if (m_vertex_buffers[m_current_buffer_idx].tail == 0)
						m_vertex->next = 0;

					for (u32 i = m_vertex->head; i < m_vertex->tail; i++)
					{
						GSVector4i* RESTRICT vert_ptr = (GSVector4i*)&m_vertex->buff[i];
						GSVector4i v = vert_ptr[1];
						v = v.xxxx().u16to32().sub32(m_xyof);
					...
					m_dirty_gs_regs = m_env_buffers[i].m_dirty_regs;
				...
				//DevCon.Warning("Picking buffer %d vertex tail %d index tail %d TME %d TBP0 0x%x draw %d", m_current_buffer_idx, m_vertex->tail, m_index->tail, m_env.PRIM.TME, m_env.CTXT[m_env.PRIM.CTXT].TEX0.TBP0, s_n);
				return true;
			}
		}
...
__forceinline void GSState::VertexKick(u32 skip)
{
	...
	if (m_recent_buffer_switch && m_index->tail > n && ((m_vertex->tail + 1) - m_vertex->head) == n)
	{
		...
		if (m_used_buffers_idx > 1)
		{
			...
			for (u32 i = m_index->buff[n]; i < m_index->tail - n; i += n)
			{
				if (v[m_index->buff[i]].XYZ.X == m_v.XYZ.X && v[m_index->buff[i]].XYZ.Y == m_v.XYZ.Y)
				{
					FlushBuffers(false);
					ResetDrawBufferIdx();
					break;
