...
void GSDrawingContext::Dump(const std::string& filename)
{
	...
	fprintf(fp,
		...
		"\tCSA:%u\n"
		"\tCLD:%u\n\n",
		TEX0.TBP0, TEX0.TBW, TEX0.PSM, psm_str(TEX0.PSM), TEX0.TW, static_cast<uint32_t>(TEX0.TH), TEX0.TCC, TEX0.TFX, TEX0.CBP, TEX0.CPSM, psm_str(TEX0.CPSM), TEX0.CSM,TEX0.CSA, TEX0.CLD);
	...
	fprintf(fp,
		...
		"\tPSM:0x%x (%s)\n"
		"\tFBMSK:0x%x\n\n",
		FRAME.FBP * 32, FRAME.FBW, FRAME.PSM, psm_str(FRAME.PSM), FRAME.FBMSK);
	...
	fprintf(fp,
		...
		"\tPSM:0x%x (%s)\n"
		"\tZMSK:%u\n\n",
		ZBUF.ZBP * 32, ZBUF.PSM, psm_str(ZBUF.PSM), ZBUF.ZMSK);
