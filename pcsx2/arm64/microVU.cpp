...
void mVUreset(microVU& mVU, bool resetReserve)
{
	...
	u8* cache_end = (mVU.index ? SysMemory::GetVU1RecEnd() : SysMemory::GetVU0RecEnd());
	armSetAsmPtr(mVU.cache, cache_end - mVU.cache, nullptr);
	armStartBlock();
	mVUdispatcherAB(mVU);
	mVUdispatcherCD(mVU);
	...
	mVU.prog.x86start = reinterpret_cast<u8*>(Common::AlignUpPow2(reinterpret_cast<uptr>(dispatcher_end), 16));
