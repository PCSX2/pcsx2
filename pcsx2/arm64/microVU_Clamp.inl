...
// Note on NaNs: minps/maxps return the second operand when either is NaN. FMINNM/FMAXNM return the
// number when one operand is NaN, so with the constant as second operand the results match x86.
	...
	if (((!clampE && CHECK_VU_OVERFLOW(mVU.index)) || (clampE && bClampE)) && mVU.regAlloc->checkVFClamp(reg.Id))
	{
		switch (xyzw)
		{
			case 1: case 2: case 4: case 8:
				// Only lane 0 is modified, like minss/maxss.
				armAsm->Ldr(a64::s31, a64::MemOperand(RMVUCONST, mVUconst(maxvals)));
				armAsm->Fminnm(a64::s30, reg.S(), a64::s31);
				armAsm->Ldr(a64::s31, a64::MemOperand(RMVUCONST, mVUconst(minvals)));
				armAsm->Fmaxnm(a64::s30, a64::s30, a64::s31);
				mVUinsLane(reg, 0, mVUScratch1, 0);
				break;
			default:
				armAsm->Fminnm(reg.V4S(), reg.V4S(), mVUloadConst(mVUconst(maxvals)));
				armAsm->Fmaxnm(reg.V4S(), reg.V4S(), mVUloadConst(mVUconst(minvals)));
				break;
		}
	}
}
...
void mVUclamp2(microVU& mVU, const xmm& reg, const xmm& regT1in, int xyzw, bool bClampE = 0)
{
	if (((!clampE && CHECK_VU_SIGN_OVERFLOW(mVU.index)) || (clampE && bClampE && CHECK_VU_SIGN_OVERFLOW(mVU.index))) && mVU.regAlloc->checkVFClamp(reg.Id))
	{
		...
		armAsm->Smin(reg.V4S(), reg.V4S(), mVUloadConst(ss ? mVUconst(sse4_maxvals_ss) : mVUconst(maxvals)));
		armAsm->Umin(reg.V4S(), reg.V4S(), mVUloadConst(ss ? mVUconst(sse4_minvals_ss) : mVUconst(minvals)));
		return;
	}
	else
