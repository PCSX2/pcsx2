...
void doIbit(mV)
{
	if (mVUup.iBit)
	{
		...
		{
			armAsm->Ldr(gprT1.W(), mVUmem(&curI));
			armAsm->Str(gprT1.W(), mVUmem(&mVU.getVI(REG_I)));
		}
		else
		{
			u32 tempI;
			...
			mVUstoreImm32(mVUmem(&mVU.getVI(REG_I)), tempI);
		}
		incPC(1);
	}
}
...
void doSwapOp(mV)
{
	if (mVUinfo.backupVF && !mVUlow.noWriteVF)
	{
		...
		mVUmovReg(t2, t1); // Backup VF reg
		mVU.regAlloc->clearNeeded(t1);

		mVUopL(mVU, 1);

		const xmm& t3 = mVU.regAlloc->allocReg(mVUlow.VF_write.reg, mVUlow.VF_write.reg, 0xf, 0);
		// Swap new and old values of the register
		mVUmovReg(mVUScratch1, t2);
		mVUmovReg(t2, t3);
		mVUmovReg(t3, mVUScratch1);
...
__fi void handleBadOp(mV, int count)
{
	...
	if (mVUinfo.isBadOp)
	{
		...
		armAsm->Mov(a64::w0, mVU.prog.cur->idx);
...
void mVUtestCycles(microVU& mVU, microFlagCycles& mFC)
{
	...
	if (EmuConfig.Gamefixes.VUSyncHack)
		armAsm->Subs(a64::w8, a64::w8, mVUcycles); // Running behind, make sure we have time to run the block
	else
		armAsm->Subs(a64::w8, a64::w8, 1); // Running ahead, make sure cycles left are above 0

	armAsm->B(&skip, a64::pl);

	mVUcopyPipelineState(mVU, &mVUpBlock->pState);

	if (EmuConfig.Gamefixes.VUSyncHack || EmuConfig.Gamefixes.FullVU0SyncHack)
		mVUstoreImm32(mVUmem(&mVU.regs().nextBlockCycles), mVUcycles);
	mVUendProgram(mVU, &mFC, 0);

	armAsm->Bind(&skip);

	mVUrmw32(mVUmem(&mVU.cycles), [&](const a64::Register& r) { armAsm->Sub(r, r, mVUcycles); });
}
...
// Sets up the assembler for the outermost mVUcompile() call. Nested calls (block linking)
// keep emitting into the same buffer.
struct mVUCodeScope
{
	microVU& mVU;
	bool owner;

	explicit mVUCodeScope(microVU& m)
		: mVU(m)
		, owner(!armHasBlock())
	{
		if (owner)
		{
			u8* end = mVU.prog.x86end + (mVUcacheSafeZone * _1mb);
			armSetAsmPtr(mVU.prog.x86ptr, end - mVU.prog.x86ptr, nullptr);
			armStartBlock();
		}
	}
...
void* mVUcompile(microVU& mVU, u32 startPC, uptr pState)
{
	...
	for (; x < endCount; x++)
	{
		...
		if (mVUup.mBit)
		{
			mVUrmw32(mVUmem(&mVU.regs().flags), [](const a64::Register& r) { armAsm->Orr(r, r, VUFLAG_MFLAGSET); });
		}
		...
		if (!mVUinfo.isBdelay && !mVUlow.branch) //T/D Bit on branch is handled after the branch, branch delay slots are executed.
		{
			...
			else if (mVUup.mBit && !mVUup.eBit && !mVUinfo.isEOB)
			{
				...
				{
					mVUstoreImm32(mVUmem(lpS), cpS[0]);
				}
				incPC(2);
				mVUsetupRange(mVU, xPC, false);
				if (EmuConfig.Gamefixes.VUSyncHack || EmuConfig.Gamefixes.FullVU0SyncHack)
