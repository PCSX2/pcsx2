...
//
// Parallel clone of pcsx2/x86/microVU_Analyze.inl. This pass is fully arch-neutral:
// it operates only on the IR (microOp/microIR/microVFreg/microVIreg) and the
// pipeline-state key (microRegInfo) via the macro layer in aVU_Misc.h — it makes NO
// emitter calls — so it ports unchanged from x86. Included by aVU.cpp.
