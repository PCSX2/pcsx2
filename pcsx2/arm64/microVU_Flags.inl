...
__fi void mVUdivSet(mV)
{
	if (mVUinfo.doDivFlag)
	{
		...
		armAsm->And(getFlagReg(sFLAG.write).W(), getFlagReg(sFLAG.write).W(), 0xfff3ffff);
		armAsm->Ldr(a64::w8, mVUmem(&mVU.divFlag));
		armAsm->Orr(getFlagReg(sFLAG.write).W(), getFlagReg(sFLAG.write).W(), a64::w8);
	}
}
...
__fi void mVUsetupFlags(mV, microFlagCycles& mFC)
{
	...
	if (doSFlagInsts && __Status)
	{
		...
		auto mov = [](const x32& dst, const x32& src) {
			if (dst != src)
				armAsm->Mov(dst.W(), src.W());
		};
		if (sortRegs == 1)
		{
			mov(gprF0, getFlagReg(bStatus[0]));
			mov(gprF1, getFlagReg(bStatus[1]));
			mov(gprF2, getFlagReg(bStatus[2]));
			mov(gprF3, getFlagReg(bStatus[3]));
		}
		else if (sortRegs == 2)
		{
			mov(gprT1, getFlagReg (bStatus[3]));
			mov(gprF0, getFlagReg (bStatus[0]));
			mov(gprF1, getFlagReg2(bStatus[1]));
			mov(gprF2, getFlagReg2(bStatus[2]));
			mov(gprF3, gprT1);
		}
		else if (sortRegs == 3)
		{
			int gFlag = (bStatus[0] == bStatus[1]) ? bStatus[2] : bStatus[1];
			mov(gprT1, getFlagReg (gFlag));
			mov(gprT2, getFlagReg (bStatus[3]));
			mov(gprF0, getFlagReg (bStatus[0]));
			mov(gprF1, getFlagReg3(bStatus[1]));
			mov(gprF2, getFlagReg4(bStatus[2]));
			mov(gprF3, gprT2);
		}
		else
		{
			const x32& temp3 = mVU.regAlloc->allocGPR();
			mov(gprT1, getFlagReg(bStatus[0]));
			mov(gprT2, getFlagReg(bStatus[1]));
			mov(temp3, getFlagReg(bStatus[2]));
			mov(gprF3, getFlagReg(bStatus[3]));
			mov(gprF0, gprT1);
			mov(gprF1, gprT2);
			mov(gprF2, temp3);
	...
	if (doMFlagInsts && __Mac)
	{
		...
		mVUshufPS(xmmT1, xmmT1, shuffleMac);
	...
	if (doCFlagInsts && __Clip)
	{
		...
		armAsm->Ldr(xmmT2.Q(), mVUmem(mVU.clipFlag));
		mVUshufPS(xmmT2, xmmT2, shuffleClip);
		armAsm->Str(xmmT2.Q(), mVUmem(mVU.clipFlag));
	}
}
