...
static __fi bool mfifoGIFrbTransfer()
{
	...
	if ((gifch.madr + mfifoqwc * 16) > (dmacRegs.mfifoRingEnd() + 16))
	{
		uint s1 = ((dmacRegs.mfifoRingEnd() + 16) - gifch.madr) >> 4;
		uint s2 = (mfifoqwc - s1);
	...
	else
	{
		...
		gifch.madr = dmacRegs.mfifoWrapAddr(gifch.madr);
	}
...
static __fi bool mfifoGIFchain()
{
	...
	if ((gifch.madr >= dmacRegs.rbor.ADDR) && (gifch.madr <= dmacRegs.mfifoRingEnd()))
	{
...
void mfifoGIFtransfer(int qwc)
{
	...
	if (gifch.qwc == 0)
	{
		...
		gifch.tadr = dmacRegs.mfifoWrapAddr(gifch.tadr);
		...
		switch (ptag->ID)
		{
			case TAG_REFE:		// Refe - Transfer Packet According to ADDR field
				gifch.tadr = dmacRegs.mfifoWrapAddr(gifch.tadr + 16);
				gifstate = GIF_STATE_DONE;									//End Transfer
				break;

			case TAG_CNT:		// CNT - Transfer QWC following the tag.
				gifch.madr = dmacRegs.mfifoWrapAddr(gifch.tadr + 16);					//Set MADR to QW after Tag
				gifch.tadr = dmacRegs.mfifoWrapAddr(gifch.madr + (gifch.qwc << 4));		//Set TADR to QW following the data
			...
			case TAG_NEXT:		// Next - Transfer QWC following tag. TADR = ADDR
			{
				...
				gifch.madr = dmacRegs.mfifoWrapAddr(gifch.tadr + 16);					//Set MADR to QW following the tag
			...
			case TAG_REFS:		// Refs - Transfer QWC from ADDR field (Stall Control)
				gifch.tadr = dmacRegs.mfifoWrapAddr(gifch.tadr + 16);					//Set TADR to next tag
				gifstate = GIF_STATE_READY;
				break;

			case TAG_END:		// End - Transfer QWC following the tag
				gifch.madr = dmacRegs.mfifoWrapAddr(gifch.tadr + 16);					//Set MADR to data following the tag
				gifch.tadr = dmacRegs.mfifoWrapAddr(gifch.madr + (gifch.qwc << 4));		//Set TADR to QW following the data
				gifstate = GIF_STATE_DONE;									//End Transfer
				break;
			}
