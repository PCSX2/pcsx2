...
{
	// TODO: Seems it's actually able to set it but I don't have
	// actual physical drive right now
}
...
bool IOCtlSrc::ReadSectors2048(u32 sector, u32 count, u8* buffer) const
{
	...
	if (GetMediaType() == -1)
	{ // always read CD via ioctl
		dk_cd_read_t desc;
		memset(&desc, 0, sizeof(dk_cd_read_t));
		desc.sectorArea = kCDSectorAreaUser;
		desc.sectorType = kCDSectorTypeMode1;
		for (u32 i = 0; i < count; ++i)
		{
			desc.offset = (sector + i) * 2048ULL;
			desc.buffer = buffer + i * 2048;
			desc.bufferLength = 2048;
			ioctl(m_device, DKIOCCDREAD, &desc);
			bytes_read += desc.bufferLength;
		}
	}
	else
	{
		bytes_read = pread(m_device, buffer, bytes_to_read, sector * 2048ULL);
	}
...
bool IOCtlSrc::ReadCDInfo()
{
	...
	int desc_count = CDTOCGetDescriptorCount(toc);
