...
{
	chd_file* chd;
	ChdCoreFileWrapper* coreWrapper = new ChdCoreFileWrapper(fp.get());
	// libchdr will take ownership of coreWrapper, and will close/free it on failure
	chd_error err = chd_open_core_file(coreWrapper->GetCoreFile(), CHD_OPEN_READ, nullptr, &chd);
	if (err == CHDERR_NONE)
	{
		// coreWrapper should manage fp
		coreWrapper->SetFileOwner(true);
		fp.release();
		return chd;
	}
	...
	coreWrapper = new ChdCoreFileWrapper(fp.get());
	// Now try re-opening with the parent.
	err = chd_open_core_file(coreWrapper->GetCoreFile(), CHD_OPEN_READ, parent_chd, &chd);
	if (err != CHDERR_NONE)
	{
		Console.Error(fmt::format("Failed to open CHD '{}': {}", filename, chd_error_string(err)));
		Error::SetString(error, chd_error_string(err));
