...
static CDVD_SourceType	m_CurrentSourceType = CDVD_SourceType::CDVDsrc_NoDisc;

void CDVDsys_SetFile( CDVD_SourceType srctype, const wxString& newfile )
{
	m_SourceFilename[int(srctype)] = newfile;
...
{
	return m_SourceFilename[int(srctype)];
}
...
void CDVDsys_ChangeSource( CDVD_SourceType type )
{
	...
	{
		case CDVD_SourceType::CDVDsrc_Iso:
			CDVD = &CDVDapi_Iso;
		break;

		case CDVD_SourceType::CDVDsrc_NoDisc:
			CDVD = &CDVDapi_NoDisc;
		break;

		case CDVD_SourceType::CDVDsrc_Plugin:
			CDVD = &CDVDapi_Plugin;
		break;
...
bool DoCDVDopen()
{
	...
	int CurrentSourceType = static_cast<int>(m_CurrentSourceType);
	int ret = CDVD->open( !m_SourceFilename[CurrentSourceType].IsEmpty() ?
		static_cast<const char*>(m_SourceFilename[CurrentSourceType].ToUTF8()) : nullptr);
