	...
	: BaseApplicableConfigPanel_SpecificConfig( parent )
{
	...
	*this += m_check_LimiterDisable | StdExpand();
	...
	s_spins += Label(_("Slow Motion Adjust:") + wxString(L"\t") + wxGetApp().GlobalAccels->findKeycodeWithCommandId("Framelimiter_SlomoToggle").toTitleizedString()) | StdExpand();
	s_spins += 5;
	...
	s_spins += Label(_("Turbo Adjust:") + wxString(L"\t") + wxGetApp().GlobalAccels->findKeycodeWithCommandId("Framelimiter_TurboToggle").toTitleizedString()) | StdExpand();
	s_spins += 5;
	...
	: BaseApplicableConfigPanel_SpecificConfig( parent )
{
	const RadioPanelItem FrameskipOptions[] =
	{
		...
		//  Implement custom hotkeys (Tab) with translatable string intact + not blank in GUI.  
		RadioPanelItem(
			_("Skip only on Turbo, to enable press") + wxString(L"\t") + wxGetApp().GlobalAccels->findKeycodeWithCommandId("Framelimiter_TurboToggle").toTitleizedString()
		),
		//  Implement custom hotkeys (Shift + F4) with translatable string intact + not blank in GUI.  
		RadioPanelItem(
			_("Constant skipping") + wxString(L"\t") + wxGetApp().GlobalAccels->findKeycodeWithCommandId("Frameskip_Toggle").toTitleizedString(),
			wxEmptyString,
