...
void MainEmuFrame::Menu_ConfigPlugin_Click(wxCommandEvent& event)
{
	...
	if (pid == PluginId_PAD)
	{
		...
		if (g_Conf->EmuOptions.EnableRecordingTools && g_InputRecordingControls.isEmulationAndRecordingPaused())
		{
			g_InputRecordingControls.Resume();
...
{
	const bool initiallyPaused = g_InputRecordingControls.isEmulationAndRecordingPaused();
	if (!initiallyPaused)
		g_InputRecordingControls.Pause();
...
void MainEmuFrame::Menu_Recording_Play_Click(wxCommandEvent& event)
{
	...
	if (!g_InputRecording.Play(path)) // Failed to Play
	{
		if (g_InputRecording.IsRecordingActive())
			Menu_Recording_Stop_Click(event);
		if (!initiallyPaused)
	...
	m_menuRecording.FindChildItem(MenuId_Recording_New)->Enable(!g_InputRecording.IsRecordingActive());
	m_menuRecording.FindChildItem(MenuId_Recording_Stop)->Enable(g_InputRecording.IsRecordingActive());
