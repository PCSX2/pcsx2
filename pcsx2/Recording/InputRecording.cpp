...
{
	MTGS::RunOnGSThread([](bool is_recording = g_InputRecording.getControls().isRecording(), std::string filename = g_InputRecording.getData().getFilename(), u32 current_frame = g_InputRecording.getFrameCounter(),
							u32 total_frames = g_InputRecording.getData().getTotalFrames(), u32 frame_count = /*g_InputRecording.getStartingFrame() + */g_InputRecording.getFrameCounterStateless(), u32 undo_count = g_InputRecording.getData().getUndoCount()) {
		g_InputRecordingData.data_lock.lock();
		...
		g_InputRecordingData.data_lock.unlock();
	});
}
