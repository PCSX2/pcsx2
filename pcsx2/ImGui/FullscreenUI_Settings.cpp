...
void FullscreenUI::DrawGraphicsSettingsPage(SettingsInterface* bsi, bool show_advanced_settings)
{
	...
	const bool extended_upscales =
		((is_vk || is_ogl) && GetEffectiveBoolSetting(bsi, "EmuCore/GS", "ExtendedUpscalingMultipliers", false));
	size_t max_shown_multiplier = extended_upscales ? std::size(s_resolution_options) : 12;
	const bool game_settings = IsEditingGameSettings(bsi);
