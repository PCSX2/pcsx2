...
void ImGuiFullscreen::DrawInputDialog()
{
			...
			ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove))
	{
		...
		// Check if we're in a numeric input field
		bool is_numeric_input = s_input_dialog_message.find("gigabytes") != std::string::npos;

		// Filter callback for InputText (only for numeric fields)
		...
		static int filter_type = 0; // 0 = no filter, 1 = numeric
		if (is_numeric_input) filter_type = 1;
		else filter_type = 0;

		ImGuiInputTextFlags flags = is_numeric_input ? ImGuiInputTextFlags_CallbackCharFilter : ImGuiInputTextFlags_None;
		ImGui::InputText("##input", &s_input_dialog_text, flags, is_numeric_input ? input_callback : nullptr, &filter_type);
