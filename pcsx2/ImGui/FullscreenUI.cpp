...
void FullscreenUI::CloseCoverDownloaderWindow()
{
	if (s_cover_downloader_thread)
	{
		...
		{
			std::lock_guard<std::mutex> lock(s_cover_downloader_mutex);
			was_downloading = s_cover_downloader_downloading;
			if (was_downloading)
				s_cover_downloader_downloading = false;
		}
...
void FullscreenUI::CoverDownloaderThreadFunc(const std::vector<std::string>& urls)
{
	...
	{
		std::lock_guard<std::mutex> lock(s_cover_downloader_mutex);
		if (!s_cover_downloader_status.empty() && s_cover_downloader_status.find("Error:") != std::string::npos)
		{
...
{
	ImGui::SetNextWindowSize(LayoutScale(1000.0f, 725.0f));
	ImGui::SetNextWindowPos(ImGui::GetIO().DisplaySize * 0.5f, ImGuiCond_Always, ImVec2(0.5f, 0.5f));
	...
	if (ImGui::BeginPopupModal(FSUI_ICONSTR(ICON_FA_DOWNLOAD, "Cover Downloader"), &s_cover_downloader_open, flags))
	{
		...
		ImGui::TextWrapped("${title}: %s", FSUI_CSTR("Title of the game."));
		ImGui::TextWrapped("${filetitle}: %s", FSUI_CSTR("Name component of the game's filename."));
		ImGui::TextWrapped("${serial}: %s", FSUI_CSTR("Serial of the game."));
		ImGui::TextWrapped("%s https://www.example-not-a-real-domain.com/covers/${serial}.jpg", FSUI_CSTR("Example:"));

		ImGui::SetCursorPosY(ImGui::GetCursorPosY() + LayoutScale(14.0f));
		...
		// URLs input section
		{
			...
			const float text_box_height = LayoutScale(80.0f);
			...
			ImGui::SetCursorPosY(ImGui::GetCursorPosY() + LayoutScale(16.0f));
		...
		ImGui::TextWrapped("%s", FSUI_CSTR("By default, the downloaded covers will be saved with the game's serial to ensure covers do not break with GameDB changes and that titles with multiple regions do not conflict. If this is not desired, you can check the \"Use Title File Names\" box above."));
...

		{
			...
			else
			{
				...
				{
					// Check if URLs are valid before starting
					...
					{
						if (url.find("${title}") != std::string::npos ||
							url.find("${filetitle}") != std::string::npos ||
							url.find("${serial}") != std::string::npos)
						{
					...
					{
						ShowToast(FSUI_STR("Error"), FSUI_STR("URL template must contain at least one of ${title}, ${filetitle}, or ${serial}."), 5.0f);
					}
					else
					{
