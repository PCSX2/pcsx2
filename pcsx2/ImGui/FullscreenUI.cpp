...
void FullscreenUI::DrawPatchesOrCheatsSettingsPage(bool cheats)
{
	...
	if (cheats)
	{
		...
		if (patch_list.empty())
		{
			...
			//	debug button for creating pnach file if none exists, also extends to copy path to clipboard for accesibility
			if (MenuButton(ICON_FA_MAGIC " Create Default Pnach", "creates a default pnach file for debug purposes.", true))
			{
				FILE*		file{};
				auto		fmtCRC	= fmt::format("{:08X}", s_game_settings_entry->crc);					//	format crc to hex
				std::string path	= EmuFolders::Cheats + FS_OSPATH_SEPARATOR_STR;							//	format path string
				std::string extent	= path + fmtCRC + ".pnach";												//	append path with file extension


				file = fopen(extent.c_str(), "r");															//	open file for reading if it exists
				if (file)																				
					return;																					//	file exists , overwrite prevented

				file = fopen(extent.c_str(), "w");															//	open file in write mode , effectively creating a new file
				if (file)
				{
					fclose(file);																			//	file created
					FileSystem::WriteStringToFile(extent.c_str(), 
						fmt::format("// Serial: {}\n// Region: {}\n// CRC: {}\ngametitle={}\ncomment=File generated by PCSX2",
							s_game_settings_entry->serial,
							GameList::RegionToString(s_game_settings_entry->region), 
							fmtCRC,
							s_game_settings_entry->title
					).c_str());																				//	write to the file
					CopyTextToClipboard("pnach path copied to clipboard.", path);							//	copy path to clipboard
				}
			}
		}
		else
	...
	struct PatchGroup
	{
		bool bUncategorizedContents = false;
		std::vector<std::string>		headers;			//	Total Headers
		std::vector<Patch::PatchInfo>	pInfoList;			//	All Patches
	};
	PatchGroup s_patch_group;

	//	Organize patch list content
	for (const Patch::PatchInfo& pi : patch_list)
	{
		auto header = pi.group;
		s_patch_group.pInfoList.push_back(pi);
		if (header != "")
		{
			auto result = find(s_patch_group.headers.begin(), s_patch_group.headers.end(), header);
			if (result == s_patch_group.headers.end())
				s_patch_group.headers.push_back(header);
		}	
		else
			s_patch_group.bUncategorizedContents = true;

		/*
			! TEST [ DEBUG ] 
		*/
		// [DEBUG TEST]	Get Content Header 
		//	size_t pos = pi.name.find("\\");
		//	if (pos > 0)
		//	{
		//		std::string header = pi.name.substr(0, pos);
		//		auto result = std::find(s_patch_group.headers.begin(), s_patch_group.headers.end(), header);
		//		if (result == s_patch_group.headers.end())
		//			s_patch_group.headers.push_back(header);
		//	}
	...
	//	- non content headers
	if (s_patch_group.bUncategorizedContents)
	{
		for (auto patches : s_patch_group.pInfoList)
		{
			if (patches.group != "")
				continue; //	skip members containing a group name

			const auto enable_it = std::find(enable_list.begin(), enable_list.end(), patches.name);
			bool state = (enable_it != enable_list.end());
			if (ToggleButton(patches.name.c_str(), patches.description.c_str(), &state, master_enable))
			{
				if (state)
				{
					bsi->AddToStringList(section, Patch::PATCH_ENABLE_CONFIG_KEY, patches.name.c_str());
					enable_list.push_back(patches.name);
				}
				else
				{
					bsi->RemoveFromStringList(section, Patch::PATCH_ENABLE_CONFIG_KEY, patches.name.c_str());
					enable_list.erase(enable_it);
				}
	...
	//	- content headers
	for (auto groups : s_patch_group.headers)
	{
		//	Content Headers
		if (ImGui::CollapsingHeader(groups.c_str()))
		{
			for (auto patches : s_patch_group.pInfoList)
			{
				//	individual patches for each content header
				if (patches.group != groups)
					continue;	// skip none matchin group name

				const auto enable_it = std::find(enable_list.begin(), enable_list.end(), patches.simple_name);

				bool state = (enable_it != enable_list.end());
				if (ToggleButton(patches.simple_name.c_str(), patches.description.c_str(), &state, master_enable))
				{
					if (state)
					{
						bsi->AddToStringList(section, Patch::PATCH_ENABLE_CONFIG_KEY, patches.simple_name.c_str());
						enable_list.push_back(patches.simple_name);
					}
					else
					{
						bsi->RemoveFromStringList(section, Patch::PATCH_ENABLE_CONFIG_KEY, patches.simple_name.c_str());
						enable_list.erase(enable_it);
					}
