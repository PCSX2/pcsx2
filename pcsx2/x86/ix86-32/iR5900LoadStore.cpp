...
void recLDL()
{
	...
	else
	{
		...
		t2reg = vtlb_DynGenReadQuad(64, -1);
	}
...
void recSDL()
{
	...
	else
	{
		...
		xForwardJE8 skip;
			xADD(calleeSavedReg1d, 1);
			int t2reg = vtlb_DynGenReadQuad(64, -1);
			...
			// ensure temps don't get allocated to rtreg
			rtreg = _allocGPRtoXMMreg(rtreg, _Rt_, MODE_READ);
