...
namespace VMManager
{
	...
	static void DoSaveState(const char* filename, s32 slot_for_message, bool zip_on_thread, bool backup_old_state, std::function<void(std::string)> error_callback);
	static void ZipSaveState(std::unique_ptr<ArchiveEntryList> elist,
		...
		s32 slot_for_message, std::function<void(std::string)> error_callback);
...
void VMManager::Shutdown(bool save_resume_state)
{
	...
	if (!GSDumpReplayer::IsReplayingDump() && save_resume_state)
	{
		...
		{
			DoSaveState(resume_file_name.c_str(), -1, true, false, [](std::string error) {
...
void VMManager::DoSaveState(const char* filename, s32 slot_for_message, bool zip_on_thread, bool backup_old_state, std::function<void(std::string)> error_callback)
{
	if (GSDumpReplayer::IsReplayingDump())
	{
...
void VMManager::SaveState(
	const char* filename, bool zip_on_thread, bool backup_old_state, std::function<void(std::string)> error_callback)
{
	if (MemcardBusy::IsBusy())
	{
...
void VMManager::SaveStateToSlot(s32 slot, bool zip_on_thread, std::function<void(std::string)> error_callback)
{
	...
	auto callback = [error_callback = std::move(error_callback), slot](std::string error) {
