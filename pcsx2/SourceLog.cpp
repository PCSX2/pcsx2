...
//
// EDIT NOTE: File extended to support optional per-channel separate log files and to
// fully format messages duplicated to those files.
...
TraceLogPack TraceLogging; // EDIT: global instances unchanged but now support separate files
ConsoleLogPack ConsoleLogging;

// -------- TraceLogFile implementation ---------------------------------
// NOTE: Default 'm_separate_files_enabled' changed to true so that separate channel
// files are generated without needing an additional configuration toggle. If desired
// this can be wired to a user setting and default reverted to false later.
TraceLogFile::TraceLogFile() : m_file(nullptr, [](std::FILE* f){ return f ? std::fclose(f) : 0; }), m_separate_files_enabled(true) {}
...
bool TraceLogFile::OpenSeparateFile(const std::string& log_name)
{
	...
	// Compose a filename based on log name (prefix) inside logs directory. EDIT: simple sanitation.
	std::string sanitized = log_name;
	for (char& c : sanitized) if (!(std::isalnum(static_cast<unsigned char>(c)) || c=='_' || c=='-')) c = '_';
...
void TraceLogFile::Write(const char* text)
{
	if (!m_separate_files_enabled)
		return;
	if (!m_file)
		return; // should have been opened lazily
	if (text && *text)
	{
		std::fputs(text, m_file.get());
		std::fputc('\n', m_file.get());
		std::fflush(m_file.get()); // EDIT: immediate flush for debugging convenience
	}
}
...
void TraceLog::OpenSeparateFileIfNeeded() const
{
	...
	if (m_trace_file.IsOpen())
		return;
	m_trace_file.OpenSeparateFile(Descriptor.Prefix); // EDIT: lazy open on first write
}

// Helper for formatting variadic args twice (log + per-file) without duplicating prefix logic.
// EDIT: Added to ensure per-channel file receives fully formatted message text.
static std::string FormatTraceLine(const LogDescriptor& desc, const char* fmt, va_list args)
{
	...
	if (required > 0)
	{
		...
		std::vsnprintf(formatted.data(), static_cast<size_t>(required) + 1, fmt, args); // EDIT: single-pass formatting
	}
	else
	{
...
bool TraceLog::Write(ConsoleColors color, const char* fmt, ...) const
{
	...
	Log::Write(LOGLEVEL_TRACE, color, line); // EDIT: consistent formatting path
...
static const LogDescriptor
	...
	LD_IOP_GPU = {"GPU", "GPU", "Detailed logging for the PS1 GPU emulation layer (IOP)."}; // EDIT: Added descriptor for new GPU log
...
TraceLogPack::IOP_PACK::IOP_PACK()
	...
	, CDVD(LD_IOP_CDVD)
	, MDEC(LD_IOP_MDEC)
	, GPU(LD_IOP_GPU) // EDIT: initialize new GPU trace log
{
}
...
void TraceLogPack::SetSeparateFilesEnabled(bool enabled)
{
	...
	IOP.GPU.SetSeparateFilesEnabled(enabled); // EDIT: include GPU in toggle
}
...
void TraceLogPack::CloseAllSeparateFiles()
{
	...
	IOP.GPU.CloseSeparateFile(); // EDIT: include GPU in close
}
